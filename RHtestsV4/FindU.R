...
FindU <- function(InSeries, output, station, MissingValueCode = -99, p.lev = 0.95, Iadj = 10000, Mq = 10, Ny4a = 0, GUI = FALSE) {
    ...
    cat(file = ofileSout, paste(" Ignore changepoints -> trend0 =", round(beta0, 6), "(", round(otmp$betaL, 6), ",", round(otmp$betaU, 6), ")", "(p=", round(otmp$p.tr, 
        4), "); cor=", round(otmp$cor, 4), "(", round(otmp$corl, 4), ",", round(otmp$corh, 4), ")\n"), append = T)
    ...
    if (PFx.mx < PFx95L) {
        ...
        while (tt) {
            ...
            # if(Debug) { cat(file=flog,paste('Niter',Niter,'new Ips:',length(Ips0),'\n'),append=T) cat(file=flog,c(Ips0,'\n'),append=T)
            # cat(file=flog,paste('Niter',Niter,'old Ns',Ns,' '),append=T) } finished search for possible new changepoints, start estimate the p-value of the Ips0 series and
            # find the most significant changepoint Iseg.mx
            tt1 <- TRUE
            while (tt1) {
                PFx.mx <- (-9999)
                Iseg.mx <- 0
        ...
        # finish finding any possible new changepoints start to delete the least significant changepoint if it is insignificant
    ...
    # itmp1<-cbind(EB1,Icy) itmp2<-cbind(1:N,Imd) colnames(itmp2)<-c('idx','Icy') itmp<-merge(itmp1,itmp2,by='Icy') EBfull<-itmp[order(itmp[,'idx']),'EB1']
    # EEB<-mean(EB1,na.rm=T)
    adj <- oout$Y0 + EBfull
    ...
    cat(paste("#steps= ", Ns, "; trend=", round(oout$trend, 6), "(", round(oout$betaL, 6), ",", round(oout$betaU, 6), ") (p=", round(oout$p.tr, 4), "); cor=", round(cor, 
    ...
    pdata[ooflg] <- oout$Y0
    plot(1:dim(ori.itable)[1], pdata, type = "l", xlab = "", ylab = "", ylim = c(min(oout$Y0, oout$meanhat), max(oout$Y0, oout$meanhat)), xaxt = "n", col = "black", 
    ...
    plot(1:dim(ori.itable)[1], pdata, type = "l", xlab = "", ylab = "", ylim = c(min(oout$Y0 + EBfull, oout$meanhat + EEB), max(oout$Y0 + EBfull, oout$meanhat + 
    ...
    if (Ns == 0) {
        ...
        for (i in 1:Ns) {
            ...
            cat(paste("PMF : c=", sprintf("%4.0f", Ic), "; (Time ", sprintf("%10.0f", IY0[Ic]), "); Type= 1; p=", sprintf("%10.4f", prob), "(", sprintf("%6.4f", 
                probL), "-", sprintf("%6.4f", probU), ")", "; PFmax=", sprintf("%10.4f", PFx), "; CV95=", sprintf("%10.4f", PFx95), "(", sprintf("%10.4f", PFx95l), 
