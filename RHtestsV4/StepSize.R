...
StepSize <- function(InSeries, InCs, output, station, MissingValueCode = -99, p.lev = 0.95, Iadj = 10000, Mq = 10, Ny4a = 0, GUI = FALSE) {
    ...
    if (length(itmp) >= 2) {
        ...
        if (sum(is.na(Ips)) > 0 | !identical(Ips, sort(Ips))) {
            ...
            for (i in 1:Ns) InsertMessagesTxt(main.txt.out, paste("StepSize: Ips read in from ", basename(InCs), "error:", Ips[i]), format = TRUE)
    ...
    par(mfrow = c(2, 1), cex.main = 0.8, cex.lab = 0.8, cex.axis = 0.8, cex = 0.8)
    ...
    plot(1:dim(ori.itable)[1], pdata, type = "l", xlab = "", ylab = "", ylim = c(min(otmp$Y0 + EBfull, otmp$meanhat + EBfull), max(otmp$Y0 + EBfull, otmp$meanhat + 
        EBfull)), xaxt = "n", col = "black", lwd = 0.5, main = "Base series and regression fit")
    axis(side = 1, at = ats, labels = labels)
    ...
    if (Ns == 0) {
        ...
        for (i in 1:Ns) {
            ...
            cat(paste(sprintf("%1.0f", Id), " ", sprintf("%-4.4s", Idc), sprintf("%10.0f", IY0[Ic]), " (", sprintf("%6.4f", probL), "-", sprintf("%6.4f", probU), 
                ")", sprintf("%6.3f", plev), sprintf("%10.4f", PFx), " (", sprintf("%10.4f", PFx95L), "-", sprintf("%10.4f", PFx95U), ")\n", sep = ""), file = ofileIout, 
                append = TRUE)
            
            
            #################################################### write '_Fstat.txt'
            
            cat(paste("PMF : c=", sprintf("%4.0f", Ic), "; (Time ", sprintf("%10.0f", IY0[Ic]), "); Type= ", sprintf("%4.0f", Id), "; p=", sprintf("%10.4f", prob), 
                "(", sprintf("%10.4f", probL), "-", sprintf("%10.4f", probU), "); PFmax=", sprintf("%10.4f", PFx), "; CV95=", sprintf("%10.4f", PFx95), "(", sprintf("%10.4f", 
                  PFx95L), "-", sprintf("%10.4f", PFx95U), "); Nseg=", sprintf("%4.0f", Nseg), "\n", sep = ""), file = ofileSout, append = T)
        }
    }
    if (GUI) 
        return(0) else {
