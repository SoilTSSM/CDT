...
StepSize.dlyPrcp <- function(InSeries, InCs, output, station, MissingValueCode = -99, pthr = 0, p.lev = 0.95, Iadj = 10000, Mq = 10, Ny4a = 0, GUI = FALSE) {
    ...
    if (length(itmp) < 2) {
        ...
        if (sum(is.na(Ips)) > 0 | !identical(Ips, sort(Ips))) {
            ...
            for (i in 1:Ns) InsertMessagesTxt(main.txt.out, paste("StepSize.dlyprcp: Ips read in from ", basename(InCs), "error:", Ips[i]), format = TRUE)
    ...
    # round(otmp$p.tr,4),'); lambda=',round(lambda,2),'; cor=', round(cor,4),'(',round(corL,4),',',round(corU,4),') p=',
    # round(pcor,4),'\n'),file=ofileSout,append=T)
    ...
    # ofileDout<-paste(output,'_adj_F.dat',sep='') write.table(odata,file=ofileDout,na=MissingValueCode,col.names=F,row.names=F)
    ...
    if (Ns > 0) {
        ...
        for (Iseg in 1:Ns) {
            # plot Y1~meanhat I1<-max(c(1,(itable[Ips[Iseg],1]-180))) Ic<-itable[Ips[Iseg],1] I2<-min(c(Nall,(itable[Ips[Iseg],1]+180)))
            ...
            # tflg<-is.na(p2data[I1:I2,3])==F lines(c(I1:I2)[tflg],p2data[I1:I2,3][tflg],col='red',lwd=1.5)
