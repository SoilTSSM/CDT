...
QMadj.GaussianDLY.wRef <- function(Bseries, Rseries, InCs, output, station, station_ref, fsuffix = "DLY", MissingValueCode = -99, Iadj = 10000, Mq = 20, Ny4a = 5) {
    ...
    pdata[owflg] <- Base + EB
    plot(1:dim(ori.bdata)[1], pdata, type = "l", xlab = "", ylab = "", ylim = c(min(Base + EB), max(Base + EB)), xaxt = "n", col = "black", lwd = 0.5, main = "a. Base series and Mean-adjusted  base series")
    axis(side = 1, at = ats, labels = labels)
    # pdata[owflg]<-otmp$meanhat+mean(EB1) lines(1:dim(ori.bdata)[1],pdata,col='red')
    ...
    pdata[owflg] <- B
    plot(1:dim(ori.bdata)[1], pdata, type = "l", xlab = "", ylab = "", ylim = c(min(B), max(B)), xaxt = "n", col = "black", lwd = 0.5, main = "b. QM-adjusted base series")
    axis(side = 1, at = ats, labels = labels)
