...
FindUD.dlyPrcp <- function(InSeries, InCs, output, station, MissingValueCode = -99, pthr = 0, p.lev = 0.95, Iadj = 10000, Mq = 10, Ny4a = 0, GUI = FALSE) {
    ...
    if (length(itmp) < 2) {
        ...
        for (i in 1:Ns.ini) {
            ...
            it <- match(ymdtmp, IY0)
            if (!is.na(it)) 
                Ips.ini[i] <- it else Ips.ini[i] <- max(c(1:N)[IY0 <= ymdtmp])
        ...
        if (sum(is.na(Ips.ini)) > 0 | !identical(Ips.ini, sort(Ips.ini))) {
            ...
            for (i in 1:Ns.ini) InsertMessagesTxt(main.txt.out, paste("FindUD.dlyprcp: Ips read in from ", basename(InCs), "error:", Ips[i]), format = TRUE)
            # ErrorMSG<<-paste(get('ErrorMSG',env=.GlobalEnv),Ips[i]) ErrorMSG<<-paste(get('ErrorMSG',env=.GlobalEnv),'\n\n')
            if (!GUI) 
                cat(ErrorMSG)  #
            return(-1)
        }
    ...
    for (dr in c(1, 0.5, 0.2, 0.1)) {
        ...
        for (lr in 1:length(lrs)) {
            ...
            # cat(paste('ind=',ind,'lambda=',lambda,'Ns=',Ns,'likelihood=',round(LH,1), 'lambda.max=',lambda.max,'\n'))
        }
        # end of detection
        if (dr > 0.1) {
    ...
    cat(paste("Ignore changepoints -> trend0 =", round(beta0, 6), "(p=", round(dtmp$p.tr, 4), "); cor=", round(dtmp$cor, 4), "(", round(dtmp$corl, 4), ",", round(dtmp$corh, 
    ...
    # ofileDout<-paste(output,'_adj_UD.dat',sep='') write.table(odata,file=ofileDout,na=MissingValueCode,col.names=F,row.names=F)
    ...
    if (Ns == 0) {
        ...
        for (i in 1:Ns) {
            ...
            cat(paste(sprintf("%1.0f", Ids[i]), " ", sprintf("%-4.4s", Idc), sprintf("%10.0f", IY0[Ic]), " (", sprintf("%6.4f", probL), "-", sprintf("%6.4f", probU), 
