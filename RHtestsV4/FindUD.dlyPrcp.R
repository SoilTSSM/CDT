...
FindUD.dlyPrcp<-function(InSeries,InCs,output,station,MissingValueCode=-99,
                pthr=0.0,p.lev=0.95,Iadj=10000,Mq=10,Ny4a=0,GUI=FALSE){
...
###write "_UDstat.txt"
  ...
  cat(paste("Input data filename:", Station,"; N=",N,"; Nday=",Nall,
      "; pthr=",pthr,"\n"), file=ofileSout)
...
# par(mar=c(3,4,3,2)+.1)
  ...
  if(Ns>0){
    ...
    for(Iseg in 1:Ns){ # plot Y1~meanhat
      ...
      plot(I1:I2,Y1[I1:I2],type="l",col="black",ylab="",xaxt="n",
  ...
  for(i in 1:length(yrs)){
    it<-match(ymd[i],IY0)
    if(!is.na(it)) ats[i]<-it
    else ats[i]<-min(c(1:N)[IY0>=ymd[i]])
  }
  plot(c(1:N),P,type="l",col="black",ylab="prcp(mm)",
       main="Original dailyP>pthr series",xaxt="n")
  axis(side=1,at=ats,labels=yrs)
  lines(c(1:N),odataP[,4],col="red")
  if(Ns>0){
    plot(c(1:N),PA,type="l",col="black",ylab="prcp(mm)",
         main="QM adjusted dailyP>pthr series",xaxt="n")
    axis(side=1,at=ats,labels=yrs)
    plot(c(1:N),PdA,type="l",col="black",ylab="prcp(mm)",
