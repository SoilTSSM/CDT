...
## Function LSmultipleRed RHtestsV4
LSmultipleRed.RHtestsV4 <- function(Y0, Ti, Ips) {
    ...
    p.tr <- pt(t.stat, df)
    betaL <- beta - qt(0.975, df) * sqrt(sigmaE2/St0)  ##RHtests only
    betaU <- beta + qt(0.975, df) * sqrt(sigmaE2/St0)  ##RHtests only
    itmp <- Y0 - beta * Ti
    mu <- rep(0, Ns + 1)
    meanhat <- mu
    for (i in 0:Ns) {
