...
StepSize.wRef <- function(Bseries, Rseries, InCs, output, station, station_ref, MissingValueCode = -99, p.lev = 0.95, Iadj = 10000, Mq = 10, Ny4a = 0, GUI = F) {
    ...
    if (Ns > 0) 
        for (i in 1:Ns) {
            ...
            cat(paste(sprintf("%1.0f", Id), " ", sprintf("%-4.4s", Idc), sprintf("%10.0f", IY0[Ic]), " (", sprintf("%6.4f", probL), "-", sprintf("%6.4f", probU), 
                ")", sprintf("%6.3f", plev), sprintf("%10.4f", PTx0), " (", sprintf("%10.4f", PTx95L), "-", sprintf("%10.4f", PTx95U), ")\n", sep = ""), file = ofileIout, 
                append = TRUE)
            
            
            #################################################### write '_Fstat.txt'
            
            cat(paste("PMT : c=", sprintf("%4.0f", Ic), "; (Time ", sprintf("%10.0f", IY0[Ic]), "); Type= ", sprintf("%4.0f", Id), "; p=", sprintf("%10.4f", prob), 
                "(", sprintf("%10.4f", probL), "-", sprintf("%10.4f", probU), "); PTmax=", sprintf("%10.4f", PTx0), "; CV95=", sprintf("%10.4f", PTx95), "(", sprintf("%10.4f", 
                  PTx95L), "-", sprintf("%10.4f", PTx95U), "); Nseg=", sprintf("%4.0f", Nseg), "\n", sep = ""), file = ofileSout, append = T)
        }
    ...
    cat(paste("Ignore changepoints -> trend0 =", round(beta0, 6), "(", round(dtmp$betaL, 6), ",", round(dtmp$betaU, 6), ") (p=", round(dtmp$p.tr, 4), "); cor=", 
        round(cor, 4), "(", round(corL, 4), ",", round(corU, 4), ")\n\n"), file = ofileSout, append = TRUE)
    ...
    while (tt) {
        ...
        if (Ns == 0) {
            ...
            if (is.na(VEB)) 
                tt <- FALSE else {
                if (max(abs(EB0 - EB1)) < VEB/1000 | Niter > 20) 
                  tt <- FALSE
            }
            EB0 <- EB1
        }
    ...
    par(mfrow = c(3, 1))
    par(mar = c(3, 4, 3, 2) + 0.1, mgp = c(1, 0.5, 0), cex.main = 0.8, cex.lab = 0.8, cex.axis = 0.8, cex = 0.8)
    ...
    cat(paste("#steps= ", Ns, "; trend=", round(otrend, 6), "(p=", round(p.tro, 4), "); cor=", round(corout[1], 4), "(", round(corout[2], 4), ",", round(corout[3], 
    ...
    if (sum(is.na(ori.bdata[, 4]) == F & Imd1 == 229) > 0) {
        ...
        for (jth in 1:length(Ids.leapo)) {
            kth <- Ids.leapo[jth]
            if (Ns > 0) {
                k1th <- if (odata[kth - 1, 2] %in% IY0[Ips0]) 
                  (kth + 1) else (kth - 1)
            } else k1th <- kth - 1
