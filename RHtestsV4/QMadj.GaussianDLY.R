...
QMadj.GaussianDLY <- function(InSeries, InCs, output, station, fsuffix = "DLY", MissingValueCode = -99, Iadj = 10000, Mq = 10, Ny4a = 0, GUI = FALSE) {
    ...
    if (length(itmp) >= 2) {
        ...
        for (i in 1:Ns) {
            ...
            if (ymdtmp == as.integer(ymdtmp/100) * 100) 
                ymdtmp <- ymdtmp + 15
        ...
        if (sum(is.na(Ips)) > 0 | !identical(Ips, sort(Ips))) {
            ...
            for (i in 1:Ns) InsertMessagesTxt(main.txt.out, paste("QMadj.Gaussian: Ips read in from ", basename(InCs), "error:", Ips[i]), format = TRUE)
    ...
    # oR<-Y1-otmp$meanhat oR[2:N]<-oR[2:N]-oR[1:(N-1)]*cor Ehat<-mean(otmp$meanhat)
    ...
    plot(1:dim(ori.itable)[1], pdata, type = "l", xlab = "", ylab = "", ylim = c(min(otmp$Y0, otmp$meanhat), max(otmp$Y0, otmp$meanhat)), xaxt = "n", col = "black", 
        lwd = 0.3, main = "Base anomaly series and regression fit")
    ...
    plot(1:dim(ori.itable)[1], pdata, type = "l", xlab = "", ylab = "", ylim = c(min(otmp$Y0 + EBfull, otmp$meanhat + EBfull), max(otmp$Y0 + EBfull, otmp$meanhat + 
        EBfull)), xaxt = "n", col = "black", lwd = 0.3, main = "Base series and regression fit")
    axis(side = 1, at = ats, labels = labels)
    ...
    pdata[olflg] <- adj
    plot(1:dim(ori.itable)[1], pdata, type = "l", xlab = "", ylab = "", ylim = c(min(c(adj, B)), max(c(adj, B))), xaxt = "n", col = "black", lwd = 0.3, main = "Mean-adjusted base series")
    axis(side = 1, at = ats, labels = labels)
    
    pdata[olflg] <- B
    plot(1:dim(ori.itable)[1], pdata, type = "l", xlab = "", ylab = "", ylim = c(min(c(adj, B)), max(c(adj, B))), xaxt = "n", col = "black", lwd = 0.3, main = "QM-adjusted base series")
    axis(side = 1, at = ats, labels = labels)
