...
QMadjGaussian <- function(P, Ips, Mq, Iseg.adj, Nadj) {
    ...
    if (Nt <= 12) 
        Ln <- 5 else Ln <- 20
    if (Mq <= 0) 
        Mq <- min(floor(Nseg.mn/Ln), 100) else Mq <- min(floor(Nseg.mn/Ln), Mq)
    if (Mq > 100) 
        Mq <- 100
    if (Mq <= 0) 
        Mq <- 1
    ...
    for (k in 1:(Mq + 2)) {
        if (Iseg.adj > 1) 
            for (i in 1:(Iseg.adj - 1)) Adj[i, k] <- sum(EPa[i:(Iseg.adj - 1), k]) - sum(EPb[i:(Iseg.adj - 1), k])
        if (Iseg.adj <= Ns) 
            for (i in (Iseg.adj + 1):(Ns + 1)) Adj[i, k] <- sum(EPb[Iseg.adj:(i - 1), k]) - sum(EPa[Iseg.adj:(i - 1), k])
    }
    
    osmean <- c(1:(Mq + 2))  # for plot purpose
    for (Iseg in c(1:(Ns + 1))) osmean <- cbind(osmean, Fcat[Iseg, ] - Fd, Adj[Iseg, ])
    # osmean<-cbind(osmean,Fcat[Iseg,]-Fd,EP[Iseg.adj,]-EP[Iseg,]) output osmean is a 2*(Ns+1)+1 by Mq+2 matrix
    ...
    for (i in 1:(Ns + 1)) {
        ...
        F[i, 1:Nseg] <- irank/Nseg
        if (i == Iseg.adj) 
            PA[I1:I2] <- P[I1:I2] else {
            dx <- Fcat[i, ] - Fd
            fdx <- Adj[i, ]
            # fdx<-EP[Iseg.adj,]-EP[i,]
            if (Mq == 1) 
                fdx[1] <- fdx[2]
            fdx2 <- splineN(dx, fdx, 2e+30, 2e+30)
...
QMadjDLY <- function(P, Ips, Mq, Iseg.adj) {
    ...
    if (Mq <= 0) 
        Mq <- min(floor(Nseg.mn/20), 100) else Mq <- min(floor(Nseg.mn/20), Mq)
    if (Mq > 100) 
        Mq <- 100
    if (Mq <= 0) 
        Mq <- 1
    ...
    osmean <- c(1:(Mq + 2))  # for plot purpose
    for (Iseg in c(1:(Ns + 1))) osmean <- cbind(osmean, Fcat[Iseg, ] - Fd, EP[Iseg.adj, ] - EP[Iseg, ])
    ...
    while (tflg) {
        ...
        for (i in 1:Ns) {
            ...
            for (k in 2:(Mq + 1)) {
                ...
                Mp2 <- floor(Nseg) * Fcat[i, k]
                if (abs(Mp1 - round(Mp1)) < 1e-09) 
                  Mp1 <- round(Mp1)
                if (abs(Mp2 - round(Mp2)) < 1e-09) 
                  Mp2 <- round(Mp2)
                Mp1 <- floor(Mp1)
                Mp2 <- floor(Mp2)
        ...
        if (tflg == FALSE) 
            {
                ...
                if (Iseg.adj > 1) 
                  AdjM[1:(Iseg.adj - 1)] <- rev(cumsum(EYd[(Iseg.adj - 1):1]))
                if (Iseg.adj <= Ns) 
                  AdjM[(Iseg.adj + 1):(Ns + 1)] <- -cumsum(EYd[Iseg.adj:Ns])
                ...
                for (k in 1:(Mq + 2)) {
                  if (Iseg.adj > 1) 
                    for (i in 1:(Iseg.adj - 1)) Adj[i, k] <- sum(EBa[i:(Iseg.adj - 1), k]) - sum(EBb[i:(Iseg.adj - 1), k])
                  if (Iseg.adj <= Ns) 
                    for (i in (Iseg.adj + 1):(Ns + 1)) Adj[i, k] <- sum(EBb[Iseg.adj:(i - 1), k]) - sum(EBa[Iseg.adj:(i - 1), k])
                }
                ...
                for (i in 1:(Ns + 1)) {
                  ...
                  if (i == Iseg.adj) 
                    ...
                    if (remain < 0) 
                      DB[DB < 0] <- DB[DB < 0] + remain * Nseg * DB[DB < 0]/sDBne else DB[DB > 0] <- DB[DB > 0] + remain * Nseg * DB[DB > 0]/sDBpo
...
adjDLYp <- function(P, Ips, Mq, Iseg.adj, Ptr.mx, R, Ncat.min, Nadj) {
    ...
    if (Mq <= 0) 
        Mq <- min(floor(Nseg.mn/Ncat.min), 100) else Mq <- min(floor(Nseg.mn/Ncat.min), Mq)
    if (Mq > 100) 
        Mq <- 100
    if (Mq <= 0) 
        Mq <- 1
    ...
    for (i in 1:(Ns + 1)) {
        ...
        F[i, 1:Nseg] <- irank/Nseg
        if (i == Iseg.adj) 
            PA[I1:I2] <- P[I1:I2] - Ptr.mx + R[I1:I2] else {
            dx <- Fcat[i, ] - Fd
            # fdx<-EP[Iseg.adj,]-EP[i,]
            fdx <- Adj[i, ]
            if (Mq == 1) 
