...
FindUD.wRef <- function(Bseries, Rseries, InCs, output, station, station_ref, MissingValueCode = -99, p.lev = 0.95, Iadj = 10000, Mq = 10, Ny4a = 0, GUI = F) {
    ...
    if (length(itmp) < 2) {
        ...
        if (sum(is.na(Ips)) > 0 | !identical(Ips, sort(Ips))) {
            InsertMessagesTxt(main.txt.out, paste("FindUD.wRef: Ips read in from ", basename(InCs), "error!"), format = TRUE)
            # cat(paste('FindUD.wRef: Ips read in from ',basename(InCs),'error!'),'\n') ErrorMSG<<-paste('FindUD.wRef: Ips read in from ',basename(InCs),'error!\n',
            # get('ErrorMSG',env=.GlobalEnv),'\n') if(!GUI) cat(ErrorMSG)
            return(-1)
        }
        Ns.ini <- Ns
    ...
    if (Ns == 0) {
        # search for 1st possible changepoint
        Pk0 <- Pk.PMT(N)
        ...
        if (probU < plev) {
            ...
            if (!GUI) {
                # cat('PMT finds the series to be homogeneous!\n')
                InsertMessagesTxt(main.txt.out, paste("PMT finds the series to be homogeneous", station))
                return(0)
            } else {
                InsertMessagesTxt(main.txt.out, paste("PMT finds the series to be homogeneous", station))
                # cat(paste('PMT finds the series',Station,'to be homogeneous!'),'\n') ErrorMSG<<-paste('PMT finds the series',Station,'to be homogeneous!\n',
                # get('ErrorMSG',env=.GlobalEnv),'\n',sep='')
                return(0)
            }
        } else {
    ...
    while (tt) {
        ...
        tt1 <- TRUE
        while (tt1) {
            # check and insert new break points
            if (length(Ips0) == 0) 
                tt1 <- FALSE else {
                probU.mx <- (-1)
                probL.mx <- (-1)
                ...
                PTx95L.mx <- 0.001
                for (i in 1:length(Ips0)) {
                  # search Ips1 series, find max prob
                  Ips1 <- sort(c(Ips, Ips0[i]))
                  Ic <- Ips0[i]
                  id <- match(Ic, Ips1)
                  if (id == length(Ips1)) {
    ...
    while (tt0) {
        while (tt) {
            # delete changepoints which are not significant
            tt <- FALSE
            probL.mn <- 9999
            Iseg.mn <- 0
            for (i in 1:Ns) {
                # check all changepoints check those un-documented
                if (Ids[i] == 0) 
                  {
                    I0 <- if (i == 1) 
                      0 else Ips[i - 1]
    ...
    for (i in 1:Ns) {
        ...
        if (Id == 0) {
            ...
            # type-1 changepoints
            if (PTx0 < PTx95L) 
                Idc <- "No  " else if (PTx0 >= PTx95L & PTx0 < PTx95U) 
                Idc <- "?   " else if (PTx0 >= PTx95U) 
                Idc <- "Yes "
        }
        ...
        cat(paste(sprintf("%1.0f", as.numeric(Id)), " ", sprintf("%-4.4s", Idc), sprintf("%10.0f", IY0[Ic]), " (", sprintf("%6.4f", probL), "-", sprintf("%6.4f", 
            probU), ")", sprintf("%6.3f", plev), sprintf("%10.4f", PTx0), " (", sprintf("%10.4f", PTx95L), "-", sprintf("%10.4f", PTx95U), ")\n", sep = ""), file = ofileIout, 
            append = TRUE)
        
        #################################################### write '_UDstat.txt'
        cat(paste("PMT : c=", sprintf("%4.0f", Ic), "; (Time ", sprintf("%10.0f", IY0[Ic]), "); Type=", sprintf("%4.0f", as.numeric(Id)), "; p=", sprintf("%10.4f", 
            prob), "(", sprintf("%10.4f", probL), "-", sprintf("%10.4f", probU), "); PTmax=", sprintf("%10.4f", PTx0), "; CV95=", sprintf("%10.4f", PTx95), "(", 
            sprintf("%10.4f", PTx95L), "-", sprintf("%10.4f", PTx95U), "); Nseg=", sprintf("%4.0f", Nseg), "\n", sep = ""), file = ofileSout, append = TRUE)
    }
    ...
    # write.table(cbind(IY0,round(oY0,4),round(omuDif,4),round(Wo,4)), file=ofileRout,col.names=F,row.names=F)
    ...
    cat(paste("Ignore changepoints -> trend0 =", round(beta0, 6), "(", round(dtmp$betaL, 6), ",", round(dtmp$betaU, 6), ") (p=", round(dtmp$p.tr, 4), "); cor=", 
        round(cor, 3), "(", round(corL, 3), ",", round(corU, 3), ")\n\n"), file = ofileSout, append = TRUE)
    ...
    if (Ns > 0) {
        # Rb<-Base-otmp$trend*Ti+EB QMout<-QMadjGaussian(Rb,Ips,Mq,Iseg.adj,Nadj)
    ...
    pdata[owflg] <- Base
    plot(1:dim(ori.bdata)[1], pdata, type = "l", xlab = "", ylab = "", ylim = c(min(Base), max(Base)), xaxt = "n", col = "black", lwd = 0.5, main = "b. De-seasonalized base series")
    axis(side = 1, at = ats, labels = labels)
    ...
    if (GUI) 
        ...
        InsertMessagesTxt(main.txt.out, paste("FindUD.wRef finished successfully for", station))
        return(0)
    }
}
