...
all_src <- list.files(path=sources_dir, pattern='.md|.Rmd',full.names=TRUE,recursive=TRUE)
rmd_files <- basename(all_src)
parent_dir <- sapply(strsplit(all_src,'/'), function(x) x[3])
all_files <- gsub('.md|.Rmd','',rmd_files)
...
# rmd_files<-c("index.Rmd","cdt_data_input.Rmd")
# parent_dir<-c("index","cdt_file")
# html_files<-c("index.html","cdt_data_input.html")
# figure_dir<-c("index_files","cdt_data_input_files")

# rmd_files<-"cdt_data_input.Rmd"
# parent_dir<-"cdt_file"
# html_files<-"cdt_data_input.html"
# figure_dir<-"cdt_data_input_files"
