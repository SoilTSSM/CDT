...
rasterData.convert_getParams <- function(parent.win, GeneralParameters){
	...
	frConvFormat <- ttklabelframe(frConversion, text = 'Data conversion', labelanchor = "nw", relief = "groove", borderwidth = 2)

	FORMAT.CONV1 <- c("NetCDF", "GeoTiff", "ESRI .hdr Labelled")

	OriginData <- tclVar()
	tclvalue(OriginData) <- switch(GeneralParameters$type.in,
									'nc' = FORMAT.CONV1[1],
									'tif' = FORMAT.CONV1[2],
									'bil' = FORMAT.CONV1[2])

	FORMAT.CONV2 <- switch(GeneralParameters$type.in,
						'nc' = c("GeoTiff", "ESRI .hdr Labelled"),
						'tif' = c("NetCDF", "ESRI .hdr Labelled"),
						'bil' = c("NetCDF", "GeoTiff"))

	ConvertData <- tclVar()
	tclvalue(ConvertData) <- switch(GeneralParameters$type.out,
									'nc' = FORMAT.CONV1[1],
									'tif' = FORMAT.CONV1[2],
									'bil' = FORMAT.CONV1[2])
	...
	txt.conv.data1 <- tklabel(frConvFormat, text = 'FROM')
	txt.conv.data2 <- tklabel(frConvFormat, text = 'TO')
	...
	tkbind(cb.in.data, "<<ComboboxSelected>>", function(){
		...
		tclvalue(textINdir) <- switch(str_trim(tclvalue(OriginData)), 
						"NetCDF" = 'Directory containing the .nc files',
						"GeoTiff" = 'Directory containing the .tif files',
						"ESRI .hdr Labelled" = 'Directory containing the .bil/.hdr files')
	})
	...
	frDirectory <- ttklabelframe(frConversion, text = 'Input/Output Directory', labelanchor = "nw", relief = "groove", borderwidth = 2)

	textDir <- switch(GeneralParameters$type.in, 
					'nc' = 'Directory containing the .nc files',
					'tif' = 'Directory containing the .nc files',
					'bil' = 'Directory containing the .bil/.hdr files')

	textINdir <- tclVar(textDir)
	dir.input <- tclVar(GeneralParameters$dir.in)
	dir.output <- tclVar(GeneralParameters$dir.out)
	...
	txt.out.dir <- tklabel(frDirectory, text = 'Directory to save converted data', anchor = 'w', justify = 'left')
	...
	frNetcdf <- ttklabelframe(frConversion, text = 'NetCDF Options', labelanchor = "nw", relief = "groove", borderwidth = 2)

	nc.name <- tclVar(GeneralParameters$nc.opts$varname)
	nc.unit <- tclVar(GeneralParameters$nc.opts$varunit)
	nc.missval <- tclVar(GeneralParameters$nc.opts$missval)
	nc.lname <- tclVar(GeneralParameters$nc.opts$longname)

	stateNC <- if(GeneralParameters$type.out == "nc") "normal" else "disabled"
	...
	tkconfigure(bt.prm.OK, command = function(){
		if(str_trim(tclvalue(dir.input))%in%c("", "NA")){
			tkmessageBox(message = "Provide the full path of the directory containing the input data", icon = "warning", type = "ok")
			tkwait.window(tt)
		}else if(str_trim(tclvalue(dir.output))%in%c("", "NA")){
			tkmessageBox(message = "Provide the full path of the directory to save results", icon = "warning", type = "ok")
			tkwait.window(tt)
		}else{
			GeneralParameters$type.in <<- switch(str_trim(tclvalue(OriginData)),
													"NetCDF" = "nc",
													"GeoTiff" = "tif",
													"ESRI .hdr Labelled" = "bil")

			GeneralParameters$type.out <<- switch(str_trim(tclvalue(ConvertData)),
													"NetCDF" = "nc",
													"GeoTiff" = "tif",
													"ESRI .hdr Labelled" = "bil")

			GeneralParameters$dir.in <<- str_trim(tclvalue(dir.input))
			GeneralParameters$dir.out <<- str_trim(tclvalue(dir.output))

			GeneralParameters$nc.opts$varname <<- str_trim(tclvalue(nc.name))
			GeneralParameters$nc.opts$varunit <<- str_trim(tclvalue(nc.unit))
			GeneralParameters$nc.opts$missval <<- as.numeric(str_trim(tclvalue(nc.missval)))
			GeneralParameters$nc.opts$longname <<- str_trim(tclvalue(nc.lname))
	...
	tkwm.transient(tt)
	tkwm.title(tt, 'Raster data format conversion')
	tkwm.deiconify(tt)
