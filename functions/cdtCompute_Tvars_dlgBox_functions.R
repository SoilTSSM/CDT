computeTvars_getParams <- function(parent.win, GeneralParameters){
	...
	temp.variable <- tclVar(GeneralParameters$variable)
	...
	status.bar.display(cb.variable, TextOutputVar, 'Select the variable to calculate')
	...
	tkconfigure(bt.prm.OK, command = function(){
		if(str_trim(tclvalue(input.Tmin))%in%c("", "NA")){
			...
			GeneralParameters$variable <<- str_trim(tclvalue(temp.variable))
