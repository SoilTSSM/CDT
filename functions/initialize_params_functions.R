...
init.params <- function(action, period){
	...
	if(action == 'homog'){
		...
		ret.params <- list(action = action, period = period, file.io = file.io, use.method = use.method, hom.opts = hom.opts, file.date.format = file.date.format, ref.series.choix = ref.series.choix, compute.var = compute.var, Adjust.method = Adjust.method, ref.series.user = ref.series.user, stn.user.choice = stn.user.choice, retpar = 0)
	}
	...
	if(action == 'rhtests'){
		...
		ret.params <- list(action = action, period = period, file.io = file.io, file.date.format = file.date.format, single.series = single.series, rhtests.pars = rhtests.pars, compute.var = compute.var, prcpdata = prcpdata, ref.series.choix = ref.series.choix, use.ref.series = use.ref.series, ref.series.user = ref.series.user, stn.user.choice = stn.user.choice, getdata = FALSE)
	}
	...
	if(action == 'qc.temp'){
		...
		AllOrOne <- 'one'
		ret.params <- list(action = action, period = period, test.tx = test.tx, file.io = file.io, use.method = use.method, file.date.format = file.date.format, parameter = parameter, AllOrOne = AllOrOne, retpar = 0)
	}
	...
	if(action == 'qc.rain'){
		...
		AllOrOne <- 'one'
		ret.params <- list(action = action, period = period, file.io = file.io, use.method = use.method, file.date.format = file.date.format, parameter = parameter, AllOrOne = AllOrOne, retpar = 0)
	}
	...
	if(action == 'coefbias.rain'){
		...
		ret.params <- list(action = action, period = period, file.io = file.io, prefix = prefix, dates.coef = dates.coef, CreateGrd = CreateGrd, params.int = params.int, new.grid = new.grid)
	}
	...
	if(action == 'merge.rain'){
		...
		ret.params <- list(action = action, period = period, file.io = file.io, prefix = prefix, dates.mrg = dates.mrg, NewGrd = NewGrd, CreateGrd = CreateGrd, blankGrd = blankGrd, params.int = params.int, new.grid = new.grid, params.mrg = params.mrg)
	}
	####dekadal update
	...
	if(action == 'merge.dekrain'){
		...
		params.mrg <- data.frame(c('interpMethod', 'RainNoRain'), c('IDW', 'GaugeSatellite'))
		names(params.mrg) <- c('Parameters', 'Values')
		ret.params <- list(action = action, period = period, file.io = file.io, prefix = prefix, dates.mrg = dates.mrg, blankGrd = blankGrd, params.int = params.int, params.mrg = params.mrg)
	}
	...
	if(action == 'down.temp'){
		...
		names(params.int) <- c('Parameters', 'Values')
		ret.params <- list(action = action, period = period, file.io = file.io, IO.file.format = IO.file.format, CreateGrd = CreateGrd, new.grid = new.grid, dates.down = dates.down, params.int = params.int)
	}
	...
	if(action == 'coefbias.temp'){
		...
		names(params.int) <- c('Parameters', 'Values')
		ret.params <- list(action = action, period = period, file.io = file.io, params.int = params.int, dates.coef = dates.coef, bias.method = bias.method, prefix = prefix)
	}
	...
	if(action == 'merge.temp'){
		...
		names(params.mrg) <- c('Parameters', 'Values')
		ret.params <- list(action = action, period = period, file.io = file.io, blankGrd = blankGrd, prefix = prefix, dates.mrg = dates.mrg, params.mrg = params.mrg)
	}
	...
	if(action == 'agg.ts'){
		...
		ret.params <- list(action = action, period = period, period1 = period1, file.io = file.io, compute.var = compute.var, data.type = data.type, file.date.format = file.date.format, datesSE = datesSE, IO.file.format = IO.file.format, netcdf.var = netcdf.var)
	}
	###########################################
	...
	if(action == 'mali.dekrain'){
		...
		rfeDownSep <- '0'
		stnDataDispo <- '1'
		ret.params <- list(action = action, period = period, file.io = file.io, dates.mrg = dates.mrg, rfeDownSep = rfeDownSep, stnDataDispo = stnDataDispo)
	}
