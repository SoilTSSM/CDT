qc.get.info.txtn <- function(parent.win, GeneralParameters){
	...
	if(as.character(GeneralParameters$use.method$Values[2]) == '0') temp.state <- 'disabled'
	if(as.character(GeneralParameters$use.method$Values[2]) == '1') temp.state <- 'normal'
	...
	if(as.character(GeneralParameters$use.method$Values[1]) == '0' & as.character(GeneralParameters$use.method$Values[3]) == '0'){
		...
		state1 <- 'disabled'
		if(as.character(GeneralParameters$use.method$Values[4]) == '0') state <- c('normal', 'normal', 'normal')
	...
	cb.file.stn1 <- ttkcombobox(fr.A12, values = unlist(listOpenFiles), textvariable = file.choix1a)
	infobulle(cb.file.stn1, 'Choose the file in the list')
	status.bar.display(cb.file.stn1, TextOutputVar, 'Choose the file containing the data to control')

	bt.file.stn1 <- tkbutton.h(fr.A12, text = "...", TextOutputVar, 'Browse file if not listed', 'Browse file if not listed') 
	...
	cb.file.stn2 <- ttkcombobox(fr.A12, values = unlist(listOpenFiles), textvariable = file.choix1b)
	infobulle(cb.file.stn2, 'Choose the file in the list')
	status.bar.display(cb.file.stn2, TextOutputVar, 'Choose the file containing the data to control')

	bt.file.stn2 <- tkbutton.h(fr.A12, text = "...", TextOutputVar, 'Browse file if not listed', 'Browse file if not listed') 
	...
	status.bar.display(cb.const.chk, TextOutputVar, 'Tmin cannot exceed Tmax for the same observing period')
...
get.param.temperature <- function(tt, GeneralParameters, state.parm){
	...
	max.stn.v <- tkentry.h(fr.C, TextOutputVar, 'Maximum number of\n neighbor stations to use', 'Maximum number of neighbor stations to use')
	win.len.v <- tkentry.h(fr.C, TextOutputVar, xperHelp, xperHelp)
