...
choose.neignbors <- function(mvar, xpos, lon, lat, elv, max.dist, min.stn){
	...
	if(ncol(Y) < min.stn){
		...
		if(!is.null(Y) & (length(idR) >= min.stn)){
			...
			delv <- if(!is.null(elv) & !is.null(elv0)) abs(elv0-elv[order(dist[idR])]) else NULL
		...
		if(uselv & !is.null(ret$delv)){
			...
			if((length(idELV) >= min.stn)){
				...
				res <- list(x = ret$x, Y = NULL, rho = NULL, dist = NULL, cov = NULL,
							msg = paste('No stations between +/-', elv.diff, 'm', 'with min.stn >=', min.stn))
			}
		}else{
			res <- list(x = ret$x, Y = ret$Y, rho = ret$rho, dist = ret$dist, cov = ret$cov,
						msg = paste(ret$msg, '. Reference series has been created without consideration elevation'))
		}
	...
	if(weight.fac == '1'){ #correlation
		if(!is.null(Y)){
			...
			if((length(idRHO) >= min.stn)){
				...
				res <- list(x = ret$x, Y = NULL, lambda = NULL,
							msg = paste('No stations with correlation coef >=',
							min.rho, 'with min.stn >=', min.stn))
			}
	...
	if(!is.null(Y)){
		if(diff.ratio == '1'){ #difference
			if(use.climato){
				...
				Y1 <- apply(Y, 2, function(x) x-mean(x, na.rm = TRUE))
				x1 <- x-mean(x, na.rm = TRUE)
			}
			ref <- x1-apply(t(lambda*t(Y1)), 1, sum, na.rm = TRUE)
			msg <- NULL
			...
			ref <- log(x1/apply(t(lambda*t(Y1)), 1, sum, na.rm = TRUE))
			ref[is.nan(ref)] <- NA
			ref[is.infinite(ref)] <- NA
			msg <- NULL
		}
		...
		if(use.climato){
			...
			fun <- match.fun(fun)
			ref <- fun(x, dates)
		}else ref <- (x-mean(x, na.rm = TRUE))/sd(x, na.rm = TRUE)
...
refSeries.creation1S <- function(cand, refs, diff.ratio, use.climato = FALSE, scale = c('daily', 'dekadal', 'monthly'), dates){
	if(diff.ratio == '1'){ #difference
		if(use.climato){
			...
			Y1 <- refs-mean(refs, na.rm = TRUE)
			x1 <- cand-mean(cand, na.rm = TRUE)
		}
		ref <- x1-Y1
	}else if(diff.ratio == '2'){ #ratio
		if(use.climato){
