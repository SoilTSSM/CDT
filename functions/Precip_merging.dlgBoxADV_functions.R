...
coefBiasGetInfoRain <- function(parent.win, GeneralParameters){
	...
	cb.period <- ttkcombobox(frInputData, values = cb.periodVAL, textvariable = file.period, width = largeur1)
	...
	tkgrid(cb.period, row = 0, column = 0, sticky = '', rowspan = 1, columnspan = 5, padx = 1, pady = 3, ipadx = 1, ipady = 1)
	...
	status.bar.display(bt.bias.interp, TextOutputVar, 'Set the parameters to interpolate the bias factor')
...
rmvBiasGetInfoRain <- function(parent.win, GeneralParameters){
	...
	tkgrid(txt.bias, row = 0, column = 0, sticky = 'we', rowspan = 1, columnspan = 2, padx = 1, pady = 5, ipadx = 1, ipady = 1)
	...
	status.bar.display(cb.bias, TextOutputVar, 'Select the method used to calculate the Bias Factors or Parameters')
	...
	tkgrid(frameRFE, row = 1, column = 0, sticky = 'we', padx = 1, pady = 1, ipadx = 1, ipady = 1)
	...
	istart.yrs <- tclVar(GeneralParameters$Adjust.Date$start.year)
	istart.mon <- tclVar(GeneralParameters$Adjust.Date$start.mon)
	istart.day <- tclVar(GeneralParameters$Adjust.Date$start.dek)
	iend.yrs <- tclVar(GeneralParameters$Adjust.Date$end.year)
	iend.mon <- tclVar(GeneralParameters$Adjust.Date$end.mon)
	iend.day <- tclVar(GeneralParameters$Adjust.Date$end.dek)
	...
	frtxtDate <- ttklabelframe(frDate, text = "Bias Correction Date Range", relief = 'groove')
	...
	tkconfigure(bt.prm.OK, command = function(){
		if(str_trim(tclvalue(dir.RFE))%in%c("", "NA")){
			...
			GeneralParameters$Adjust.Date$start.year <<- as.numeric(str_trim(tclvalue(istart.yrs)))
			GeneralParameters$Adjust.Date$start.mon <<- as.numeric(str_trim(tclvalue(istart.mon)))
			GeneralParameters$Adjust.Date$start.dek <<- as.numeric(str_trim(tclvalue(istart.day)))
			GeneralParameters$Adjust.Date$end.year <<- as.numeric(str_trim(tclvalue(iend.yrs)))
			GeneralParameters$Adjust.Date$end.mon <<- as.numeric(str_trim(tclvalue(iend.mon)))
			GeneralParameters$Adjust.Date$end.dek <<- as.numeric(str_trim(tclvalue(iend.day)))
...
coefLMGetInfoRain <- function(parent.win, GeneralParameters){
	...
	tkgrid(cb.period, row = 0, column = 0, sticky = '', rowspan = 1, columnspan = 5, padx = 1, pady = 5, ipadx = 1, ipady = 1)
	...
	frLMCoef <- tkframe(frLeft, relief = 'sunken', borderwidth = 2)
	...
	en.LMCoef.years1 <- tkentry(fr.baseLM, width = 6, textvariable = LMCoef.year1, justify = 'right')
	en.LMCoef.years2 <- tkentry(fr.baseLM, width = 6, textvariable = LMCoef.year2, justify = 'right')

	bt.LMCoef.interp <- ttkbutton(frLMCoef, text = "LMCoef Interpolations Parameters")
...
mergeGetInfoRain <- function(parent.win, GeneralParameters){
	...
	bt.LMCoef.dir <- tkbutton(frMrg, text = "...", state = stateLMCoef)
