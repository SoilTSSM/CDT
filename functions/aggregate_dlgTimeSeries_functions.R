mainDialogAggTs<-function(parent.win,gal.params){
	...
	OriginData <- tclVar()
	tclvalue(OriginData) <- ifelse(as.character(gal.params$period)=='daily',
	...
	status.bar.display(Cbperiod0,txt.stbr1,'Choose the time step of the data to convert')

	ConvertData <- tclVar()
	tclvalue(ConvertData) <- ifelse(as.character(gal.params$period1)=='dekadal',
	'Dekadal data',ifelse(as.character(gal.params$period1)=='monthly','Monthly data','Yearly data'))
	...
	status.bar.display(Cbperiod1,txt.stbr1,'Aggregate data to')
	...
	datatyp<-as.character(gal.params$data.type)
	DataType <- tclVar()
	...
	bt.opt.set<-tkbutton.h(labdatype, text="Settings",txt.stbr1,'Options setting for data','Options setting for data')
	...
	if(tclvalue(DataType)=='One station series'){
		...
		tkconfigure(bt.opt.set,state='normal',command=function(){
			gal.params<<-netcdfDataAgg(tt,gal.params,tclvalue(OriginData),tclvalue(ConvertData))
			settingdone<<-1
		})
	}
	...
	AggreFunlab<-tklabel.h(fr.A2,text='Aggregation Fun',txt.stbr1,
	...
	aggfun<-tclVar(as.character(gal.params$compute.var$Values[1]))
	...
	status.bar.display(CbAggreFun,txt.stbr1,'Function to be used to convert dekadal, monthly and yearly series')

	MissFraclab<-tklabel.h(fr.A2,text='Min.frac',txt.stbr1,
	...
	MissFracentr<-tkentry.h(fr.A2,txt.stbr1,
	'Minimum fraction of available data\nthat must be present for the time period\nto compute',
	'Minimum fraction of available data that must be present for the time period to compute')

	MissFrac<-tclVar(as.character(gal.params$compute.var$Values[2]))
	...
	status.bar.display(cb.stnfl,txt.stbr1,'Choose the file containing the data to convert')

	bt.stnfl<-tkbutton.h(fr.B1, text="...",txt.stbr1,'Browse file if not listed','Browse file if not listed')
	...
	status.bar.display(en.file.save,txt.stbr1,'Enter the full path of the file to save converted data')
	bt.file.save<-tkbutton.h(fr.B3, text="...",txt.stbr1,'or browse here','or browse here')
	...
	tkbind(Cbdatatype,"<<ComboboxSelected>>",function(){
		if(tclvalue(DataType)=='One station series'){
			...
			status.bar.display(cb.stnfl,txt.stbr1,'Enter the full path to directory containing the NetCDF data')
	...
	tkconfigure(bt.prm.OK,command=function(){
		if(tclvalue(file.stnfl)==""){
			...
			gal.params$period<<-ifelse(tclvalue(OriginData)=='Daily data','daily',
			ifelse(tclvalue(OriginData)=='Dekadal data','dekadal','monthly'))
			gal.params$period1<<-ifelse(tclvalue(ConvertData)=='Dekadal data','dekadal',
			ifelse(tclvalue(ConvertData)=='Monthly data','monthly','yearly'))

			gal.params$data.type<<-ifelse(tclvalue(DataType)=='One station series','series',
			ifelse(tclvalue(DataType)=='CDT data format','cdt','netcdf'))
			gal.params$compute.var$Values<<-c(tclvalue(aggfun),tclvalue(MissFrac))
...
netcdfDataAgg<-function(tt,gal.params,daydek,daydek1){
	...
	status.bar.display(fr.A,txt.stbr1,'Start and end date of data to convert')
	...
	if(daydek=='Daily data'){
		...
		day.val0<-tclVar(as.character(gal.params$datesSE$Values[3]))
		day.val1<-tclVar(as.character(gal.params$datesSE$Values[6]))
		stdaydek<-'normal'
	}else if(daydek=='Dekadal data'){
		day.txt<-tklabel(fr.A,text='Dek')
		day.vl0<-as.numeric(as.character(gal.params$datesSE$Values[3]))
		day.val0<-if(day.vl0>3) tclVar('3') else tclVar(day.vl0)
		day.vl1<-as.numeric(as.character(gal.params$datesSE$Values[6]))
	...
	istart.yrs<-tclVar(as.character(gal.params$datesSE$Values[1]))
	istart.mon<-tclVar(as.character(gal.params$datesSE$Values[2]))
	istart.day<-day.val0
	iend.yrs<-tclVar(as.character(gal.params$datesSE$Values[4]))
	iend.mon<-tclVar(as.character(gal.params$datesSE$Values[5]))
	iend.day<-day.val1
	...
	status.bar.display(en.inrfeff,txt.stbr1,'Enter the format of the input NetCDF files names, example: daily_19830101.nc')
	tkgrid(en.inrfeff,row=0,column=0,sticky='w')

	frB2.txt<-tklabel(fr.B2,text='Output NetCDF files names format')
	tkgrid(frB2.txt)

	outmrgff <-tclVar(as.character(gal.params$IO.file.format$Values[2]))
	...
	status.bar.display(en.outmrgff,txt.stbr1,'Enter the format of the output NetCDF files names, example: dek_19830101.nc')
	tkgrid(en.outmrgff,row=0,column=0,sticky='w')
		...
		if(daydek=='Daily data'){
			...
			status.bar.display(en.outmrgff,txt.stbr1,'Enter the format of the output NetCDF files names, example: dek_1983011.nc')
		}else if(daydek=='Dekadal data'){
			...
			infobulle(en.inrfeff,'Enter the format of the input NetCDF files names,\nexample: dek_1983011.nc')
			status.bar.display(en.inrfeff,txt.stbr1,'Enter the format of the input NetCDF files names, example: dek_1983011.nc')
			infobulle(en.outmrgff,'Enter the format of the output NetCDF files names,\nexample: mon_198301.nc')
			status.bar.display(en.outmrgff,txt.stbr1,'Enter the format of the output NetCDF files names, example: mon_198301.nc')
		}else if(daydek=='Monthly data'){
			tclvalue(inrfeff)<-"mon_%s%s.nc"
			tclvalue(outmrgff)<-"year_%s.nc"
			infobulle(en.inrfeff,'Enter the format of the input NetCDF files names,\nexample: mon_198301.nc')
			status.bar.display(en.inrfeff,txt.stbr1,'Enter the format of the input NetCDF files names, example: mon_198301.nc')
			infobulle(en.outmrgff,'Enter the format of the output NetCDF files names,\nexample: year_1983.nc')
			status.bar.display(en.outmrgff,txt.stbr1,'Enter the format of the output NetCDF files names, example: year_1983.nc')
		}
	...
	status.bar.display(varid.out.txt,txt.stbr1,'Name of the variable to be created for netcdf output')

	varid.out<-tclVar(as.character(gal.params$netcdf.var$Values[1]))
	...
	status.bar.display(varid.out.v,txt.stbr1,'Name of the variable to be created for netcdf output')
	...
	status.bar.display(longname.out.txt,txt.stbr1,'Longer name of the variable to be created for netcdf output')

	longname.out<-tclVar(as.character(gal.params$netcdf.var$Values[2]))
	...
	status.bar.display(longname.out.v,txt.stbr1,'Longer name of the variable to be created for netcdf output')
	...
	tkconfigure(bt.prm.OK,command=function(){
		gal.params$datesSE$Values<<-c(tclvalue(istart.yrs),tclvalue(istart.mon),tclvalue(istart.day),
		tclvalue(iend.yrs),tclvalue(iend.mon),tclvalue(iend.day))
		gal.params$IO.file.format$Values<<-c(tclvalue(inrfeff),tclvalue(outmrgff))
		gal.params$netcdf.var$Values<<-c(tclvalue(varid.out),tclvalue(longname.out))
		tkgrab.release(tt1)
		tkdestroy(tt1)
		tkfocus(tt)
	})
...
oneStnDataAgg<-function(top.win,gal.params,speriod){
	tt1<-tktoplevel()
	tkgrab.set(tt1)
	tkfocus(tt1)
