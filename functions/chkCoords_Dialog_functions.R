excludeOutStn<-function(parent.win,gal.params){
	...
	cb.stnfl<-ttkcombobox(fr.A1, values=unlist(file.list), textvariable=file.stnfl,width=wtkcombo)
	...
	bt.stnfl<-tkbutton.h(fr.A1, text="...",txt.stbr1,'Browse file if not listed','Browse file if not listed')
	...
	file.blkshp <- tclVar()
	tclvalue(file.blkshp) <- as.character(gal.params$file.io$Values[2])

	cb.blkshp<-ttkcombobox(fr.B1, values=unlist(file.list), textvariable=file.blkshp,width=wtkcombo)
	infobulle(cb.blkshp,'Choose the file in the list')
	status.bar.display(cb.blkshp,txt.stbr1,'File containing the ESRI shapefiles')
	bt.blkshp<-tkbutton.h(fr.B1, text="...",txt.stbr1,'Browse file if not listed','Browse file if not listed')
	...
	bt.file.save<-tkbutton.h(fr.C1, text=" ...",txt.stbr1,'or browse here','or browse here')
...
###
	...
	status.bar.display(fr.D,txt.stbr1,'Distance in km, outside the country boudanries to considere')

	bufftxt<-tklabel(fr.D,text='Distance from boundaries(km)')
	buffw <-tclVar(as.character(gal.params$buffer))
...
###
	...
	tkconfigure(bt.opt.OK,command=function(){
		if(tclvalue(file.stnfl)==""){
			...
			idsstn<-which(unlist(lapply(1:length(file.opfiles),function(j) file.opfiles[[j]][[1]]))==tclvalue(file.stnfl))
			if(length(idsstn)==0){
				tkmessageBox(message="File not found or in the wrong format",icon="warning",type="ok")
				tkwait.window(tt)
			}else{
				gal.params$file.io$Values<<-c(tclvalue(file.stnfl),tclvalue(file.blkshp),tclvalue(file.save1))
				gal.params$buffer<<-tclvalue(buffw)
