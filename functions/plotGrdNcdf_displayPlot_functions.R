...
plotOneNCDF.Map <- function(){
	don <- EnvOneNCDFPlot$ncData$map
	dataMapOp <- EnvOneNCDFPlot$ncMapOp

	## titre
	if(!dataMapOp$title$user){
		titre <- EnvOneNCDFPlot$ncData$file2plot
	...
	lines(ocrds[, 1], ocrds[, 2], lwd = EnvOneNCDFPlot$SHPOp$lwd, col = EnvOneNCDFPlot$SHPOp$col)
...
OneNCDF.Display.Map <- function(parent){
	plotIt <- function(){
		plotOneNCDF.Map()
	}

	#########
	onglet <- imageNotebookTab_open(parent, EnvOneNCDFPlot$notebookTab.dataNCMap,
				paste('Map -', EnvOneNCDFPlot$ncData$file2plot), AllOpenTabType, AllOpenTabData)
