...
AggregateDataCDT <- function(GeneralParameters){
	...
	if(filefrmt == "0"){
		...
		write.table(donne, fileout, col.names = FALSE, row.names = FALSE, quote = FALSE)
		...
		donne1[is.na(donne1)] <- -99
		write.table(donne1, fileout1, col.names = FALSE, row.names = FALSE, quote = FALSE)
		...
		donne2[is.na(donne2)] <- -99
		write.table(donne2, fileout2, col.names = FALSE, row.names = FALSE, quote = FALSE)
	}else{
	...
	outlist <- list()
	if(!is.null(miss)){
		tmp <- as.data.frame(infoheadI[miss, , drop = FALSE])
