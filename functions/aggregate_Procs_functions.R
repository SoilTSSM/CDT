...
AggregateDataCDT <- function(GeneralParameters){
	...
	for(j in 1:length(ggid)){
		...
		if(file.exists(fileopen)){
			is.rdble <- !inherits(try(donne <- readFun(fileopen, header = delimter$header, sep = delimter$sepr, skip = delimter$skip-1, na.strings = delimter$miss.val, quote = "\"'", strip.white = TRUE, colClasses = "character"), silent = TRUE), "try-error")
			if(!is.rdble){
