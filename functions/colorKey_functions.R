...
createColorkey <- function(parent.win, listCol){
	...
	listCanColor <- list()
	kcol <- if(Sys.info()["sysname"] == "Windows") 10 else 14
	...
	parseKolor <- function(vcolor){
		...
		if(length(txtcol) > 0){
			...
			if(any(!existcol)){
				InsertMessagesTxt(main.txt.out, paste('Unknown color name:',
								paste0(vtxtcol[!existcol], collapse = ", ")), format = TRUE)
				vcolor <- vcolor[-txtcol[!existcol]]
				if(length(vcolor) == 0){
					InsertMessagesTxt(main.txt.out, 'No valid color name', format = TRUE)
					return(NULL)
				}
			}
		}
	...
	txtCol_width <- if(Sys.info()["sysname"] == "Windows") 30 else 40

	yscrColor <- tkscrollbar(frEditCol, repeatinterval = 4, command = function(...) tkyview(textColor, ...))
	textColor <- tktext(frEditCol, bg = "white", yscrollcommand = function(...) tkset(yscrColor, ...),
							wrap = "word", height = 5, width = txtCol_width)
	...
	tkconfigure(saveColor, command = function(){
		...
		if(Sys.info()["sysname"] == "Windows"){
			file2save <- tclvalue(tkgetSaveFile(initialdir = getwd(), initialfile = "",
								filetypes = filetypes, defaultextension = TRUE))
		}else{
			file2save <- tclvalue(tkgetSaveFile(initialdir = getwd(),
								initialfile = "", filetypes = filetypes))
		}
