...
climatologiesCalc.plotClimMaps <- function(){
	don <- EnvClimatoCalcPlot$climdata$map
	climMapOp <- EnvClimatoCalcPlot$climMapOp

	## titre
	if(!climMapOp$title$user){
		titre2 <- if(EnvClimatoCalcPlot$climdata$Var == "CDTMEAN") "mean" else "standard deviation"
		year1 <- EnvClimatoCalcPlot$output$params$climato$start
		year2 <- EnvClimatoCalcPlot$output$params$climato$end
		titre3 <- paste0("(", year1, "-", year2, ")")

		if(EnvClimatoCalcPlot$output$params$intstep == "daily"){
			titre1 <- "Daily"
			titre4 <- "for"
		...
		if(EnvClimatoCalcPlot$output$params$intstep == "monthly"){
			titre1 <- "Monthly"
			titre4 <- "for"
			seqtime <- format(seq(as.Date('2015-1-1'), as.Date('2015-12-31'), 'month'), "%B")
		}

		ipos <- as.numeric(str_trim(tclvalue(EnvClimatoCalcPlot$climDate)))
		titre5 <- seqtime[ipos]
...
climatologiesCalc.plotClimGraph <- function(){
	TSGraphOp <- EnvClimatoCalcPlot$TSGraphOp

	if(EnvClimatoCalcPlot$output$params$data.type == "cdtstation"){
		ixy <- which(EnvClimatoCalcPlot$output$data$id == str_trim(tclvalue(EnvClimatoCalcPlot$graph$stnIDTSp)))
		if(length(ixy) == 0){
			InsertMessagesTxt(main.txt.out, "Station not found", format = TRUE)
			return(NULL)
		}
		don <- EnvClimatoCalcPlot$climdata$data[, ixy]
		idaty <- EnvClimatoCalcPlot$output$index
		EnvClimatoCalcPlot$location <- paste0("Station: ", EnvClimatoCalcPlot$output$data$id[ixy])
	}else{
		cdtdataset <- EnvClimatoCalcPlot$cdtdataset
		xlon <- cdtdataset$coords$mat$x
		xlat <- cdtdataset$coords$mat$y
		ilon <- as.numeric(str_trim(tclvalue(EnvClimatoCalcPlot$graph$lonLOC)))
		ilat <- as.numeric(str_trim(tclvalue(EnvClimatoCalcPlot$graph$latLOC)))
		...
		EnvClimatoCalcPlot$location <- paste0("Longitude: ", round(ilon, 5), ", Latitude: ", round(ilat, 5))
	}
...
climatologiesCalc.Display.Maps <- function(parent){
	...
	plotIt <- function(){
		...
		op <- par(bg = "white")
		pltusr <- climatologiesCalc.plotClimMaps()
		par(op)
		for(j in seq_along(varplot)) tclvalue(parPltCrd[[varplot[j]]]) <- pltusr$par[j]
	}

	#########
	onglet <- imageNotebookTab_open(parent, EnvClimatoCalcPlot$notebookTab.ClimMap, 'Climatology-Map', AllOpenTabType, AllOpenTabData)
	...
	tkbind(img, "<Motion>", function(W, x, y){
		if(length(ls(EnvClimatoCalcPlot)) == 0) return(NULL)
		if(is.null(EnvClimatoCalcPlot$output)) return(NULL)
		...
		if(EnvClimatoCalcPlot$output$params$data.type == "cdtstation"){
			...
			sdist <- (xyMouse$x-EnvClimatoCalcPlot$output$data$lon)^2 + (xyMouse$y-EnvClimatoCalcPlot$output$data$lat)^2
			...
			frzcoord <- ifelse(xyMouse$inout | rayondisp, '', EnvClimatoCalcPlot$output$data$id[inear])
		}else{
			frzcoord <- ""
		}
	...
	tkbind(img, "<Button-1>", function(W, x, y){
		...
		if(EnvClimatoCalcPlot$output$params$data.type == "cdtstation"){
			...
			if(!(xyMouse$inout | rayondisp)){
				tclvalue(EnvClimatoCalcPlot$graph$stnIDTSp) <- EnvClimatoCalcPlot$output$data$id[inear]
				plotTS <- TRUE
			}else plotTS <- FALSE
		}else{
			if(!xyMouse$inout){
				tclvalue(EnvClimatoCalcPlot$graph$lonLOC) <- round(xyMouse$x, 6)
				tclvalue(EnvClimatoCalcPlot$graph$latLOC) <- round(xyMouse$y, 6)
				plotTS <- TRUE
			}else plotTS <- FALSE
		}
...
climatologiesCalc.Display.Graph <- function(parent){
	plotIt <- function(){
		climatologiesCalc.plotClimGraph()
	}

	#########
	onglet <- imageNotebookTab_open(parent, EnvClimatoCalcPlot$notebookTab.ClimGraph, 'Climatology-Graph', AllOpenTabType, AllOpenTabData)
	hscale <- as.numeric(tclvalue(tkget(spinH)))
	vscale <- as.numeric(tclvalue(tkget(spinV)))
