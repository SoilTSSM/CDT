...
executeOnFindU <- function(donnees, GeneralParameters){
	...
	outdir <- file.path(GeneralParameters$IO.files$dir2save, paste('RHtests_Output',
						getf.no.ext(GeneralParameters$IO.files$Cand.file), sep = '_'), 'Outputs')
	p.lev <- GeneralParameters$rhtests.pars$p.lev
	Iadj <- GeneralParameters$rhtests.pars$Iadj
	Mq <- GeneralParameters$rhtests.pars$Mq
	Ny4a <- GeneralParameters$rhtests.pars$Ny4a
	...
	if(GeneralParameters$prcp.data$rr.data & GeneralParameters$prcp.data$rr.log){
		if(min(xval, na.rm = TRUE) > 0) Inxval <- log(xval)
	 	else Inxval <- log(xval+1)
	}else Inxval <- xval

	InSeries <- cbind(as.numeric(substr(donnees$modates, 1, 4)),
					as.numeric(substr(donnees$modates, 5, 6)), 0, Inxval)

	dirSelectStn <- file.path(outdir, sel.stn)
	dir.create(dirSelectStn, showWarnings = FALSE, recursive = TRUE)
	fileout <- file.path(dirSelectStn, sel.stn)
	ret <- FindU(InSeries, fileout, sel.stn, MissingValueCode = donnees$MissingValue,
					p.lev = p.lev, Iadj = Iadj, Mq = Mq, Ny4a = Ny4a)
	if(ret < 0){
...
executeOnFindUD <- function(donnees, GeneralParameters){
	...
	ret <- FindUD(InSeries, InCs, fileout, sel.stn, MissingValueCode = donnees$MissingValue,
					p.lev = p.lev, Iadj = Iadj, Mq = Mq, Ny4a = Ny4a)
	if(ret < 0){
...
executeOnStepSize <- function(donnees, GeneralParameters){
	...
	ret <- StepSize(InSeries, InCs, fileout, sel.stn, MissingValueCode = donnees$MissingValue,
					p.lev = p.lev, Iadj = Iadj, Mq = Mq, Ny4a = Ny4a)
	if(ret < 0){
...
executeOnQMadj <- function(donnees, GeneralParameters, choix){
	...
	rootdir <- file.path(GeneralParameters$IO.files$dir2save, paste('RHtests_Output',
						getf.no.ext(GeneralParameters$IO.files$Cand.file), sep = '_'))
	outdir <- file.path(rootdir, 'Outputs')
	adjdir <- file.path(rootdir, 'AdjustedData')
	...
	sel.stn <- tclvalue(lchoixStnFr$env$stn.choix.val)
	if(!GeneralParameters$stn.type$single.series)  xpos <- which(as.character(donnees$IDs) == sel.stn)

	dirSelectStn <- file.path(outdir, sel.stn)
	fileout <- file.path(dirSelectStn, sel.stn)

	dirAdjStn <- file.path(adjdir, sel.stn)
	dir.create(dirAdjStn, showWarnings = FALSE, recursive = TRUE)
	fileadj <- file.path(dirAdjStn, sel.stn)
	...
	if(GeneralParameters$prcp.data$rr.data & GeneralParameters$prcp.data$rr.log){
		if(!GeneralParameters$stn.type$single.series) xval0 <- donnees$datmon[, xpos]
		else xval0 <- donnees$datmon
		InSeries0 <- cbind(as.numeric(substr(donnees$modates, 1, 4)),
						as.numeric(substr(donnees$modates, 5, 6)), 0, xval0)
		ret0 <- QMadj.GaussianDLY(InSeries0, InCs, fileout, sel.stn, fsuffix = 'MON',
				MissingValueCode = donnees$MissingValue, Iadj = Iadj, Mq = Mq, Ny4a = Ny4a)
		if(ret0 < 0){
	...
	write.table(outAdjMON, paste(fileadj, '_MON.txt', sep = ''), col.names = FALSE, row.names = FALSE)

	if(GeneralParameters$period != 'monthly'){
		if(!GeneralParameters$stn.type$single.series) xval1 <- donnees$datdek[, xpos]
		else xval1 <- donnees$datdek
		InSeries1 <- cbind(as.numeric(substr(donnees$dkdates, 1, 4)),
							as.numeric(substr(donnees$dkdates, 5, 6)),
							as.numeric(substr(donnees$dkdates, 7, 7)), xval1)
		ret1 <- QMadj.GaussianDLY(InSeries1, InCs, fileout, sel.stn, fsuffix = 'DEK',
								MissingValueCode = donnees$MissingValue, Iadj = Iadj, Mq = Mq, Ny4a = Ny4a)
		if(ret1 < 0){
			...
			if(GeneralParameters$prcp.data$rr.data) outadjQM1[outadjQM1 != donnees$MissingValue & outadjQM1 < 0] <- 0
			outAdjDEK <- data.frame(donnees$dkdates, outadjQM1)
			write.table(outAdjDEK, paste(fileadj, '_DEK.txt', sep = ''), col.names = FALSE, row.names = FALSE)
		}
		if(GeneralParameters$period == 'daily'){
			if(!GeneralParameters$stn.type$single.series) xval2 <- donnees$datdly[, xpos]
			else xval2 <- donnees$datdly
			InSeries2 <- cbind(as.numeric(substr(donnees$dydates, 1, 4)),
								as.numeric(substr(donnees$dydates, 5, 6)),
								as.numeric(substr(donnees$dydates, 7, 8)), xval2)
			ret2 <- QMadj.GaussianDLY(InSeries2, InCs, fileout, sel.stn, fsuffix = 'DLY',
						MissingValueCode = donnees$MissingValue, Iadj = Iad, Mq = Mq, Ny4a = Ny4a)
			if(ret2 < 0){
				...
				if(GeneralParameters$prcp.data$rr.data) outadjQM2[outadjQM2 != donnees$MissingValue & outadjQM2 < 0] <- 0
				outAdjDLY <- data.frame(donnees$dydates, outadjQM2)
				write.table(outAdjDLY, paste(fileadj, '_DLY.txt', sep = ''), col.names = FALSE, row.names = FALSE)
			}
		}
	}
	write.table(choix, paste(fileadj, '_CHOICE.txt', sep = ''), col.names = FALSE, row.names = FALSE)
...
executeOnFindU.wRef <- function(donnees, dem_data, GeneralParameters){
	...
	if(!GeneralParameters$stn.type$single.series){
		...
		if(GeneralParameters$refSeries.choix$use.elv & is.null(dem_data)) InsertMessagesTxt(main.txt.out, refseries$msg, format = TRUE)
		if(is.null(xrefs)){
		...
		station_ref <- getf.no.ext(GeneralParameters$IO.files$Ref.file)
		if(is.null(xrefs)){
	...
	if(GeneralParameters$prcp.data$rr.data & GeneralParameters$prcp.data$rr.log){
		...
		if(min(xrefs, na.rm = TRUE) > 0) Inxrefs <- log(xrefs)
	...
	dirRefSeries <- file.path(datadir, 'RefSeries_Data', sel.stn)
	dir.create(dirRefSeries, showWarnings = FALSE, recursive = TRUE)
	###reference series
	wRefseries <- data.frame(daty, round(xrefs, 4))
	reffileout <- file.path(dirRefSeries, paste(sel.stn, paste('RefS-', 'MON', '.txt', sep = ''), sep = '_'))
	write.table(wRefseries, reffileout, col.names = FALSE, row.names = FALSE)

	fileout <- file.path(dirSelectStn, sel.stn)

	ret <- FindU.wRef(InSeries, RefSeries, fileout, station_base, station_ref, MissingValueCode = donnees$MissingValue,
						p.lev = p.lev, Iadj = Iadj, Mq = Mq, Ny4a = Ny4a)
	if(ret < 0){
...
executeOnFindUD.wRef <- function(donnees, dem_data, GeneralParameters){
	...
	ret <- FindUD.wRef(InSeries, RefSeries, InCs, fileout, station_base, station_ref,
			MissingValueCode = donnees$MissingValue, p.lev = p.lev, Iadj = Iadj, Mq = Mq, Ny4a = Ny4a)
	if(ret < 0){
...
executeOnStepSize.wRef <- function(donnees, dem_data, GeneralParameters){
	...
	ret <- StepSize.wRef(InSeries, RefSeries, InCs, fileout, station_base, station_ref, MissingValueCode = donnees$MissingValue,
						p.lev = p.lev, Iadj = Iadj, Mq = Mq, Ny4a = Ny4a)
	if(ret < 0){
...
executeOnQMadj.wRef <- function(donnees, dem_data, GeneralParameters, choix){
	...
	if(GeneralParameters$prcp.data$rr.data & GeneralParameters$prcp.data$rr.log){
		...
		InSeries0 <- cbind(as.numeric(substr(donnees$modates, 1, 4)),
							as.numeric(substr(donnees$modates, 5, 6)), 0, xval0)
		RefSeries0 <- cbind(as.numeric(substr(donnees$modates, 1, 4)),
							as.numeric(substr(donnees$modates, 5, 6)), 0, xrefs0)
		ret0 <- QMadj.GaussianDLY.wRef(InSeries0, RefSeries0, InCs, fileout, station_base, station_ref, fsuffix = 'MON',
										MissingValueCode = donnees$MissingValue, Iadj = Iadj, Mq = Mq, Ny4a = Ny4a)
		if(ret0 < 0){
	...
	if(GeneralParameters$period != 'monthly'){
		...
		InSeries1 <- cbind(as.numeric(substr(donnees$dkdates, 1, 4)),
							as.numeric(substr(donnees$dkdates, 5, 6)),
							as.numeric(substr(donnees$dkdates, 7, 7)), xval1)
		RefSeries1 <- cbind(as.numeric(substr(donnees$dkdates, 1, 4)),
							as.numeric(substr(donnees$dkdates, 5, 6)),
							as.numeric(substr(donnees$dkdates, 7, 7)), xrefs1)

		wRefseries1 <- data.frame(donnees$dkdates, round(xrefs1, 4))
		reffileout <- file.path(dirRefSeries, paste(sel.stn, paste('RefS-', 'DEK', '.txt', sep = ''), sep = '_'))
		write.table(wRefseries1, reffileout, col.names = FALSE, row.names = FALSE)

		ret1 <- QMadj.GaussianDLY.wRef(InSeries1, RefSeries1, InCs, fileout, station_base, station_ref, fsuffix = 'DEK',
										MissingValueCode = donnees$MissingValue, Iadj = Iadj, Mq = Mq, Ny4a = Ny4a)
		if(ret1 < 0){
		...
		if(GeneralParameters$period == 'daily'){
			...
			InSeries2 <- cbind(as.numeric(substr(donnees$dydates, 1, 4)),
								as.numeric(substr(donnees$dydates, 5, 6)),
								as.numeric(substr(donnees$dydates, 7, 8)), xval2)
			RefSeries2 <- cbind(as.numeric(substr(donnees$dydates, 1, 4)),
								as.numeric(substr(donnees$dydates, 5, 6)),
								as.numeric(substr(donnees$dydates, 7, 8)), xrefs2)

			wRefseries2 <- data.frame(donnees$dydates, round(xrefs2, 4))
			reffileout <- file.path(dirRefSeries, paste(sel.stn, paste('RefS-', 'DLY', '.txt', sep = ''), sep = '_'))
			write.table(wRefseries2, reffileout, col.names = FALSE, row.names = FALSE)

			ret2 <- QMadj.GaussianDLY.wRef(InSeries2, RefSeries2, InCs, fileout, station_base, station_ref, fsuffix = 'DLY',
											MissingValueCode = donnees$MissingValue, Iadj = Iadj, Mq = Mq, Ny4a = Ny4a)
			if(ret2 < 0){
...
executeOnFindU.dlyPrcp <- function(donnees, GeneralParameters){
	...
	Mq <- GeneralParameters$rhtests.pars$Mq
	Ny4a <- GeneralParameters$rhtests.pars$Ny4a
	pthr <- GeneralParameters$rhtests.pars$pthr
	...
	InSeries <- cbind(as.numeric(substr(donnees$dydates, 1, 4)),
					as.numeric(substr(donnees$dydates, 5, 6)),
					as.numeric(substr(donnees$dydates, 7, 8)), xval)

	dirSelectStn <- file.path(outdir, sel.stn)
	dir.create(dirSelectStn, showWarnings = FALSE, recursive = TRUE)
	fileout <- file.path(dirSelectStn, sel.stn)
	ret <- FindU.dlyPrcp(InSeries, fileout, sel.stn, MissingValueCode = donnees$MissingValue,
						pthr = pthr, p.lev = p.lev, Iadj = Iadj, Mq = Mq, Ny4a = Ny4a)
	if(ret < 0){
...
executeOnFindUD.dlyPrcp <- function(donnees, GeneralParameters){
	...
	ret <- FindUD.dlyPrcp(InSeries, InCs, fileout, sel.stn, MissingValueCode = donnees$MissingValue,
						pthr = pthr, p.lev = p.lev, Iadj = Iadj, Mq = Mq, Ny4a = Ny4a)
	if(ret < 0){
...
executeOnStepSize.dlyPrcp <- function(donnees, GeneralParameters){
	...
	ret <- StepSize.dlyPrcp(InSeries, InCs, fileout, sel.stn, MissingValueCode = donnees$MissingValue,
							pthr = pthr, p.lev = p.lev, Iadj = Iadj, Mq = Mq, Ny4a = Ny4a)
	if(ret < 0){
...
executeOnadjDLY.dlyPrcp <- function(donnees, GeneralParameters, choix){
	...
	if(!is.null(ReturnExecResults)){
		if(length(grep('dlyPrcp', ReturnExecResults$action)) > 0){
			...
			write.table(outAdjDLYp, paste(fileadj, '_DLY.txt', sep = ''), col.names = FALSE, row.names = FALSE)
			write.table(choix, paste(fileadj, '_CHOICE.txt', sep = ''), col.names = FALSE, row.names = FALSE)
...
RHtestsDisplayPdfPlot <- function(ret){
	if(!is.null(ret)){
		fileout <- file.path(ret$StnOutDir, ret$stn)
		...
		if(ret$action == 'QMadj'){
			logRR <- GeneralParameters$prcp.data$rr.data & GeneralParameters$prcp.data$rr.log
...
formatRHtestsPreviewOutput <- function(ret, text1, text2){
	...
	todisplay <- c(paste('File:', paste(ret$stn, text1, sep = '')),
					'------------------------', ' ', file1, ' ',
					paste(rep('*', 100), collapse = ''), ' ',
					paste('File:', paste(ret$stn, text2, sep = '')),
					'------------------------', ' ', file2)
...
formatRHtestsPreviewOutput1 <- function(ret, text1, text2, text3 = NULL){
	...
	if(is.null(text3)){
		...
		todisplay <- c(paste('File:', paste(ret$stn, text1, sep = '')),
						...
						paste('File:', paste(ret$stn, text2, sep = '')),
						'------------------------', ' ', file2, ' ',
						paste('File:', paste(ret$stn, text3, sep = '')),
						'------------------------', ' ', file3)
	}
	retOut <- paste(todisplay, collapse = '\n')
	return(retOut)
...
RHtestsPreviewOutput <- function(ret){
	...
	if(ret$action == 'QMadj'){
		if(GeneralParameters$period == 'daily') retOut <- formatRHtestsPreviewOutput1(ret, '_mCs.txt',
															paste("_QMadj", 'DEK', "stat.txt", sep = ''),
															paste("_QMadj", 'DLY', "stat.txt", sep = ''))
		if(GeneralParameters$period == 'dekadal') retOut <- formatRHtestsPreviewOutput1(ret, '_mCs.txt',
															paste("_QMadj", 'DEK', "stat.txt", sep = ''))
	}
	if(ret$action == 'QMadj.wRef'){
		if(GeneralParameters$period == 'daily') retOut <- formatRHtestsPreviewOutput1(ret, '_mCs.txt',
															paste("_QMadj", 'DEK', "wRefstat.txt", sep = ''),
															paste("_QMadj", 'DLY', "wRefstat.txt", sep = ''))
		if(GeneralParameters$period == 'dekadal') retOut <- formatRHtestsPreviewOutput1(ret, '_mCs.txt',
															paste("_QMadj", 'DEK', "wRefstat.txt", sep = ''))
	}
	return(retOut)
...
RHtestsUndoChange <- function(ret){
	fileout <- paste(file.path(ret$StnOutDir, ret$stn),'_mCs.txt', sep = '')
	if(ret$action == 'FindU') file.copy(paste(file.path(ret$StnOutDir, ret$stn),'_1Cs.txt', sep = ''), fileout, overwrite = TRUE)
	if(ret$action == 'FindUD') file.copy(paste(file.path(ret$StnOutDir, ret$stn),'_pCs.txt', sep = ''), fileout, overwrite = TRUE)
	if(ret$action == 'StepSize') file.copy(paste(file.path(ret$StnOutDir, ret$stn),'_fCs.txt', sep = ''), fileout, overwrite = TRUE)
	if(ret$action == 'FindU.wRef') file.copy(paste(file.path(ret$StnOutDir, ret$stn),'_1Cs.txt', sep = ''), fileout, overwrite = TRUE)
	if(ret$action == 'FindUD.wRef') file.copy(paste(file.path(ret$StnOutDir, ret$stn),'_pCs.txt', sep = ''), fileout, overwrite = TRUE)
	if(ret$action == 'StepSize.wRef') file.copy(paste(file.path(ret$StnOutDir, ret$stn),'_fCs.txt', sep = ''), fileout, overwrite = TRUE)
	if(ret$action == 'FindU.dlyPrcp') file.copy(paste(file.path(ret$StnOutDir, ret$stn),'_1Cs.txt', sep = ''), fileout, overwrite = TRUE)
	if(ret$action == 'FindUD.dlyPrcp') file.copy(paste(file.path(ret$StnOutDir, ret$stn),'_pCs.txt', sep = ''), fileout, overwrite = TRUE)
	if(ret$action == 'StepSize.dlyPrcp') file.copy(paste(file.path(ret$StnOutDir, ret$stn),'_fCs.txt', sep = ''), fileout, overwrite = TRUE)
}

######

formatted_mCs2vec <- function(x){
	tmp <- c(substr(x, 1, 1), substr(x, 3, 6), substr(x, 7, 16), substr(x, 19, 24),
			substr(x, 26, 31), substr(x, 33, 38), substr(x, 39, 48), substr(x, 51, 60), substr(x, 62, 71))
	tmp <- str_trim(tmp)
	#tmp[tmp == ""] <- NA
	return(tmp)
}
...
RHtests_mCsFormat <- function(ret){
	fileInCs <- paste(file.path(ret$StnOutDir, ret$stn),'_mCs.txt', sep = '')
	...
	if(length(Incs) > 0){
		...
		if(nbcpt > 0){
			...
			if(nrow(tmp) == 1){
				tmp <- data.frame(matrix(c(tmp[, 1:3], paste('(', tmp[, 4], '-', tmp[, 5], ')', sep = ''),
								tmp[, 6:7], paste('(', tmp[, 8], '-',tmp[, 9], ')', sep = '')), nrow = 1))
			}else{
				tmp <- data.frame(cbind(tmp[, 1:3], paste('(', tmp[, 4], '-', tmp[, 5], ')', sep = ''),
								tmp[, 6:7], paste('(', tmp[, 8], '-', tmp[, 9], ')', sep = '')), row.names = NULL)
			}
