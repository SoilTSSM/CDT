Temp_mergeGetInfoALL <- function(parent.win, GeneralParameters){
	...
	tkgrid(cb.mrg, row = 0, column = 2, sticky = 'we', rowspan = 1, columnspan = 6, padx = 1, pady = 1, ipadx = 1, ipady = 1)
	tkgrid(bt.mrg.interp, row = 1, column = 0, sticky = 'we', rowspan = 1, columnspan = 8, padx = 1, pady = 1, ipadx = 1, ipady = 1)
	...
	frblank <- tkframe(frLeft, relief = 'sunken', borderwidth = 2)
	...
	infobulle(frtxtDate, 'Start and end date to merge temperature data')
	status.bar.display(frtxtDate, TextOutputVar, 'Start and end date to merge temperature data')
