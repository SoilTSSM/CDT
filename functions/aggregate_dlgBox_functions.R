...
AggregateInputStationData<-function(parent.win,gal.params){
...
###########################
	...
	cb.file.stn.sample<-ttkcombobox(fr.A12, values=unlist(file.list), textvariable=file.choix1,width=25)
	infobulle(cb.file.stn.sample,'Choose the file in the list')
	status.bar.display(cb.file.stn.sample,txt.stbr1,'File containing a sample of data')

	bt.file.stn.sample<-tkbutton.h(fr.A12, text="...",txt.stbr1,'Browse file if not listed','Browse file if not listed')
	...
	tkconfigure(bt.file.stn.sample,command=function(){
		...
		if(!is.null(dat.opfiles)){
			...
			tkconfigure(cb.file.stn.sample,values=unlist(file.list), textvariable=file.choix1)
			tkconfigure(cb.file.stn.info,values=unlist(file.list), textvariable=file.choix2)
		}else{
			return(NULL)
		}
	})
...
################################
	...
	cb.file.stn.info<-ttkcombobox(fr.A22, values=unlist(file.list), textvariable=file.choix2,width=25)
	infobulle(cb.file.stn.info,'Choose the file in the list')
	status.bar.display(cb.file.stn.info,txt.stbr1,'File containing the ids and coordinates of the stations')

	bt.file.stn.info<-tkbutton.h(fr.A22, text="...",txt.stbr1,'Browse file if not listed','Browse file if not listed')
...
#############################
	...
	dir.stn <-tclVar(as.character(gal.params$file.io$Values[3]))
	...
	status.bar.display(en.dir.stn,txt.stbr1,'Enter the full path to directory containing the station files')
	bt.dir.stn<-tkbutton.h(fr.A32, text="...",txt.stbr1,'or browse here','')
	...
	tkconfigure(bt.dir.stn,command=function(){
		dir4stn<-tk_choose.dir(as.character(gal.params$file.io$Values[3]), "")
...
################################
	...
	status.bar.display(en.file.save,txt.stbr1,'Enter the file to save aggregated data')
	bt.file.save<-tkbutton.h(fr.A42, text="...",txt.stbr1,'or browse here','or browse here')
...
#####################
	...
	istart.yrs<-tclVar(as.character(gal.params$StartEnd.date$Values[1]))
	istart.mon<-tclVar(as.character(gal.params$StartEnd.date$Values[2]))
	istart.day<-tclVar(as.character(gal.params$StartEnd.date$Values[3]))
	iend.yrs<-tclVar(as.character(gal.params$StartEnd.date$Values[4]))
	iend.mon<-tclVar(as.character(gal.params$StartEnd.date$Values[5]))
	iend.day<-tclVar(as.character(gal.params$StartEnd.date$Values[6]))
	...
	tkbind(cb.period,"<<ComboboxSelected>>",function(){
		if(tclvalue(file.period)=='Daily data'){
			...
			tkconfigure(day1.v,state='normal')
			tkconfigure(day2.v,state='normal')
			xend<-as.numeric(as.character(gal.params$StartEnd.date$Values[6]))
...
AggregateOutputStationData<-function(parent.win,gal.params){
	tt<-tktoplevel()
	tkgrab.set(tt)
	tkfocus(tt)
	...
	file.save1 <-tclVar(as.character(gal.params$file.io))
	...
	status.bar.display(en.file.save,txt.stbr1,'Enter the full path of the directory containing the folders Outputs, OriginalData or CorrectedData')
	bt.file.save<-tkbutton.h(frA2, text="...",txt.stbr1,'or browse here','or browse here')
	tkgrid(en.file.save,bt.file.save)
	tkgrid.configure(en.file.save,row=0,column=0,sticky='w')
	tkgrid.configure(bt.file.save,row=0,column=1,sticky='e')
	tkconfigure(bt.file.save,command=function(){
		file2save1<-tk_choose.dir(as.character(gal.params$file.io), "")
	...
	tkconfigure(bt.OK,command=function(){
		gal.params$file.io<<-tclvalue(file.save1)
		tkgrab.release(tt)
		tkdestroy(tt)
		tkfocus(parent.win)
	})
