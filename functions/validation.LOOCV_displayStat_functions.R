...
LOOCValidation.plotStatMaps <- function(){
	...
	don <- cdt.as.image(don, pts.xy = cbind(EnvLOOCValidation$opDATA$lon, EnvLOOCValidation$opDATA$lat), nx = nx, ny = ny)
...
LOOCValidation.plotGraph <- function(){
	...
	if(plotType == "Lines"){
		...
		abline(h = axTicks(2), col = "lightgray", lty = "dotted")
		# abline(v = axTicks(1), col = "lightgray", lty = "dotted")
		abline(v = axTicks.Date(EnvLOOCValidation$opDATA$temps, 1), col = "lightgray", lty = "dotted")
