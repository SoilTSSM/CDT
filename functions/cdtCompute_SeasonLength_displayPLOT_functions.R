...
SeasonLengthCalc.plotSeasLenMaps <- function(){
	don <- EnvSeasLengthCalcPlot$varData$map
	dataMapOp <- EnvSeasLengthCalcPlot$dataMapOp

	## titre
	if(!dataMapOp$title$user){
		titre <- paste("Season length:", str_trim(tclvalue(EnvSeasLengthCalcPlot$donDate)))
	...
	### shape files
	shpf <- EnvSeasLengthCalcPlot$shp
	...
	idt <- which(donDates == str_trim(tclvalue(EnvSeasLengthCalcPlot$donDate)))
	# legendLabel <- format(breaks + EnvSeasLengthCalcPlot$output$start.date[idt], '%d-%b')
	legendLabel <- breaks
	...
	lines(ocrds[, 1], ocrds[, 2], lwd = EnvSeasLengthCalcPlot$SHPOp$lwd, col = EnvSeasLengthCalcPlot$SHPOp$col)
...
SeasonLengthCalc.plotSeasLenGraph <- function(){
	TSGraphOp <- EnvSeasLengthCalcPlot$TSGraphOp

	if(EnvSeasLengthCalcPlot$output$params$data.type == "cdtstation"){
		ixy <- which(EnvSeasLengthCalcPlot$output$data$id == str_trim(tclvalue(EnvSeasLengthCalcPlot$graph$stnIDTSp)))
		if(length(ixy) == 0){
			InsertMessagesTxt(main.txt.out, "Station not found", format = TRUE)
			return(NULL)
		}
		don <- as.numeric(EnvSeasLengthCalcPlot$varData$data[, ixy])
		EnvSeasLengthCalcPlot$location <- paste0("Station: ", EnvSeasLengthCalcPlot$output$data$id[ixy])
		titre <- paste0("(", EnvSeasLengthCalcPlot$output$data$id[ixy], ")")
	}else{
		cdtdataset <- EnvSeasLengthCalcPlot$cdtdataset
		xlon <- cdtdataset$coords$mat$x
		xlat <- cdtdataset$coords$mat$y
		ilon <- as.numeric(str_trim(tclvalue(EnvSeasLengthCalcPlot$graph$lonLOC)))
		ilat <- as.numeric(str_trim(tclvalue(EnvSeasLengthCalcPlot$graph$latLOC)))
		...
		EnvSeasLengthCalcPlot$location <- paste0("Longitude: ", round(ilon, 5), ", Latitude: ", round(ilat, 5))
		titre <- ""
	}

	daty <- as.numeric(format(EnvSeasLengthCalcPlot$output$start.date, "%Y"))

	titre <- paste("Season length", titre)

	#########

	GRAPHTYPE <- str_trim(tclvalue(EnvSeasLengthCalcPlot$graph$typeTSp))
	...
	if(GRAPHTYPE == "Barplot"){
		...
		climatoAnalysis.plot.bar(daty, don, xlim = xlim, ylim = ylim, origindate = NULL,
								xlab = xlab, ylab = ylab, ylab.sub = NULL,
								title = titre, title.position = titre.pos, axis.font = 1,
...
SeasonLengthCalc.Display.Maps <- function(parent){
	...
	plotIt <- function(){
		...
		op <- par(bg = "white")
		pltusr <- SeasonLengthCalc.plotSeasLenMaps()
		par(op)
		for(j in seq_along(varplot)) tclvalue(parPltCrd[[varplot[j]]]) <- pltusr$par[j]
	}

	#########
	onglet <- imageNotebookTab_open(parent, EnvSeasLengthCalcPlot$notebookTab.dataMap, 'Season Length-Map', AllOpenTabType, AllOpenTabData)
	...
	tkbind(img, "<Motion>", function(W, x, y){
		if(length(ls(EnvSeasLengthCalcPlot)) == 0) return(NULL)
		if(is.null(EnvSeasLengthCalcPlot$output)) return(NULL)
		...
		if(EnvSeasLengthCalcPlot$output$params$data.type == "cdtstation"){
			...
			sdist <- (xyMouse$x-EnvSeasLengthCalcPlot$output$data$lon)^2 + (xyMouse$y-EnvSeasLengthCalcPlot$output$data$lat)^2
			...
			frzcoord <- ifelse(xyMouse$inout | rayondisp, '', EnvSeasLengthCalcPlot$output$data$id[inear])
		}else{
			frzcoord <- ""
		}
	...
	tkbind(img, "<Button-1>", function(W, x, y){
		...
		if(EnvSeasLengthCalcPlot$output$params$data.type == "cdtstation"){
			...
			if(!(xyMouse$inout | rayondisp)){
				tclvalue(EnvSeasLengthCalcPlot$graph$stnIDTSp) <- EnvSeasLengthCalcPlot$output$data$id[inear]
				plotTS <- TRUE
			}else plotTS <- FALSE
		}else{
			if(!xyMouse$inout){
				tclvalue(EnvSeasLengthCalcPlot$graph$lonLOC) <- round(xyMouse$x, 6)
				tclvalue(EnvSeasLengthCalcPlot$graph$latLOC) <- round(xyMouse$y, 6)
				plotTS <- TRUE
			}else plotTS <- FALSE
		}
...
SeasonLengthCalc.Display.Graph <- function(parent){
	plotIt <- function(){
		SeasonLengthCalc.plotSeasLenGraph()
	}

	#########
	onglet <- imageNotebookTab_open(parent, EnvSeasLengthCalcPlot$notebookTab.dataGraph, 'Season Length-Graph', AllOpenTabType, AllOpenTabData)
	hscale <- as.numeric(tclvalue(tkget(spinH)))
	vscale <- as.numeric(tclvalue(tkget(spinV)))
