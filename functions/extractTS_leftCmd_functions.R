...
ExtractDataPanelCmd <- function(){
	...
	##########################################
		...
		timeSteps <- tclVar()
		CbperiodVAL <- c('Hourly data', 'Daily data', 'Pentad data', 'Dekadal data', 'Monthly data')
		tclvalue(timeSteps) <- switch(GeneralParameters$in.series, 
										'hourly' = CbperiodVAL[1], 
										'daily' = CbperiodVAL[2], 
										'pentad' = CbperiodVAL[3], 
										'dekadal' = CbperiodVAL[4],
										'monthly' = CbperiodVAL[5])
		...
		tkbind(cb.fperiod, "<<ComboboxSelected>>", function(){
			OUTSeries <- c('Hourly', 'Daily', 'Pentad', 'Dekadal', 'Monthly', '3-Months', '6-Months', 'Annual')
			if(tclvalue(timeSteps) == 'Hourly data'){
				stateHR <- 'normal'
				stateDR <- 'normal'
			...
			if(tclvalue(timeSteps) == 'Daily data'){
				...
				tclvalue(DayDek.txtVar) <- 'Day'
				tclvalue(out.series) <- if(tclvalue(out.series) == 'Hourly') 'Daily' else tclvalue(out.series)
			}
			if(tclvalue(timeSteps) == 'Pentad data'){
				stateHR <- 'disabled'
				stateDR <- 'normal'
				OUTSeries <- OUTSeries[-(1:2)]
				tclvalue(DayDek.txtVar) <- 'Pen'
				tclvalue(out.series) <- if(tclvalue(out.series)%in%c('Hourly', 'Daily')) 'Pentad' else tclvalue(out.series)
	 			tclvalue(istart.day) <- if(as.numeric(tclvalue(istart.day)) > 6) 6 else tclvalue(istart.day)
				tclvalue(iend.day) <- if(as.numeric(tclvalue(iend.day)) > 6) 6 else tclvalue(iend.day)
			}
			if(tclvalue(timeSteps) == 'Dekadal data'){
				stateHR <- 'disabled'
				stateDR <- 'normal'
				OUTSeries <- OUTSeries[-(1:3)]
				tclvalue(DayDek.txtVar) <- 'Dek'
				tclvalue(out.series) <- if(tclvalue(out.series)%in%c('Hourly', 'Daily', 'Pentad')) 'Dekadal' else tclvalue(out.series)
			...
			if(tclvalue(timeSteps) == 'Monthly data'){
				stateHR <- 'disabled'
				stateDR <- 'disabled'
				OUTSeries <- OUTSeries[-(1:4)]
				tclvalue(out.series) <- if(tclvalue(out.series) %in% c('Hourly', 'Daily', 'Pentad', 'Dekadal')) 'Monthly' else tclvalue(out.series)
			}

			tkconfigure(hour1.v, state = stateHR)
			tkconfigure(hour2.v, state = stateHR)
			...
			if((tclvalue(out.series) == 'Hourly' & tclvalue(timeSteps) == 'Hourly data') |
				(tclvalue(out.series) == 'Daily' & tclvalue(timeSteps) == 'Daily data') |
				(tclvalue(out.series) == 'Pentad' & tclvalue(timeSteps) == 'Pentad data') |
				...
				if(!tclvalue(timeSteps)%in%c('Hourly data', 'Daily data')){
			...
			####
			if(tclvalue(timeSteps) == 'Hourly data'){
				OUTTypeSeries <- c('Raw Time Series', '')
				tclvalue(type.series) <- 'Raw Time Series'
			}else{
				OUTTypeSeries <- c('Raw Time Series', 'Anomalies', 'Standardized Anomalies', 'Climatologies')
				tclvalue(type.series) <- if(tclvalue(type.series) == "") 'Raw Time Series' else tclvalue(type.series)
			}
			tkconfigure(cb.typeseries, values = OUTTypeSeries)

			stateClim0 <- if(tclvalue(type.series)%in%c('Raw Time Series', '')) 'disabled' else 'normal'
			tkconfigure(en.minYear, state = stateClim0)
			tkconfigure(chk.allYears, state = stateClim0)

			stateClim1 <- if(tclvalue(timeSteps) != 'Hourly data'){
							if(tclvalue(type.series) != 'Raw Time Series' &
								tclvalue(UseAllYears) == '0') 'normal' else 'disabled'
							}else "disabled"
			tkconfigure(en.startYear, state = stateClim1)
			tkconfigure(en.endYear, state = stateClim1)

			stateClim2 <- if(tclvalue(timeSteps) != 'Hourly data'){
								if(tclvalue(timeSteps) == 'Daily data' &
									tclvalue(type.series) != 'Raw Time Series') "normal" else "disable"
							}else "disabled"
	...
	##########################################
		...
		istart.hour <- tclVar(GeneralParameters$date.range$start.hour)
		...
		iend.hour <- tclVar(GeneralParameters$date.range$end.hour)
		...
		if(GeneralParameters$in.series == 'daily'){
			...
			stateHR <- 'disabled'
			stateDR <- "normal"
			istart.day <- if(as.numeric(str_trim(GeneralParameters$date.range$start.day)) > 6) tclVar(6) else istart.day
			iend.day <- if(as.numeric(str_trim(GeneralParameters$date.range$end.day)) > 6) tclVar(6) else iend.day
		...
		mon.txt <- tklabel(frDatyR, text = 'Mon')
		day.txt <- tklabel(frDatyR, text = tclvalue(DayDek.txtVar), textvariable = DayDek.txtVar)
		hour.txt <- tklabel(frDatyR, text = 'Hour')

		yrs1.v <- tkentry(frDatyR, width = 4, textvariable = istart.yrs, justify = "left")
		mon1.v <- tkentry(frDatyR, width = 4, textvariable = istart.mon, justify = "left")
		day1.v <- tkentry(frDatyR, width = 4, textvariable = istart.day, justify = "left", state = stateDR)
		hour1.v <- tkentry(frDatyR, width = 4, textvariable = istart.hour, justify = "left", state = stateHR)

		yrs2.v <- tkentry(frDatyR, width = 4, textvariable = iend.yrs, justify = "left")
		mon2.v <- tkentry(frDatyR, width = 4, textvariable = iend.mon, justify = "left")
		day2.v <- tkentry(frDatyR, width = 4, textvariable = iend.day, justify = "left", state = stateDR)
		hour2.v <- tkentry(frDatyR, width = 4, textvariable = iend.hour, justify = "left", state = stateHR)
		...
		tkgrid(hour1.v, row = 1, column = 4, sticky = 'we', rowspan = 1, columnspan = 1, padx = 1, pady = 1, ipadx = 1, ipady = 1)
		...
		tkgrid(hour2.v, row = 2, column = 4, sticky = 'we', rowspan = 1, columnspan = 1, padx = 1, pady = 1, ipadx = 1, ipady = 1)

		tkgrid(frDatyR, row = 0, column = 0, sticky = 'we', rowspan = 1, columnspan = 4, padx = 1, pady = 1, ipadx = 1, ipady = 1)
		tkgrid(txt.month, row = 1, column = 0, sticky = 'we', rowspan = 1, columnspan = 4, padx = 1, pady = 1, ipadx = 1, ipady = 1)
		tkgrid(fr.Month, row = 2, column = 0, sticky = 'we', rowspan = 1, columnspan = 4, padx = 1, pady = 1, ipadx = 1, ipady = 1)
		...
		OUTSeries <- c('Hourly', 'Daily', 'Pentad', 'Dekadal', 'Monthly', '3-Months', '6-Months', 'Annual')
		if(GeneralParameters$in.series == 'daily') OUTSeries <- OUTSeries[-1]
		if(GeneralParameters$in.series == 'pentad') OUTSeries <- OUTSeries[-(1:2)]
		if(GeneralParameters$in.series == 'dekadal') OUTSeries <- OUTSeries[-(1:3)]
		if(GeneralParameters$in.series == 'monthly') OUTSeries <- OUTSeries[-(1:4)]
		out.series <- tclVar()
		tclvalue(out.series) <- switch(GeneralParameters$out.series$out.series, 
										...
										'dekadal' = OUTSeries[4],
										'monthly' = OUTSeries[5],
										'seasonal3' = OUTSeries[6], 
										'seasonal6' = OUTSeries[7], 
										'annual' = OUTSeries[8])
		...
		stateTSOut <- if(GeneralParameters$in.series %in% c('hourly', 'daily', 'pentad', 'dekadal', 'monthly')) "disabled" else "normal"
		...
		tkbind(cb.outTS, "<<ComboboxSelected>>", function(){
			if(tclvalue(out.series) %in% c('Hourly', 'Daily', 'Pentad', 'Dekadal', 'Monthly')){
				stateTSOut <- "disabled"
				stateMon <- "normal"
		...
		AGGREGFUN <- c("mean", "sum", "count")
		if(GeneralParameters$in.series%in%c('hourly', 'daily') & !GeneralParameters$out.series$out.series%in%c('hourly', 'daily')) AGGREGFUN <- AGGREGFUN[-3]
	...
	##########################################
		...
		if(GeneralParameters$in.series == 'hourly'){
			OUTTypeSeries <- c('Raw Time Series', '')
			type.series <- tclVar('Raw Time Series')
		}else{
			OUTTypeSeries <- c('Raw Time Series', 'Anomalies', 'Standardized Anomalies', 'Climatologies')
			type.series <- tclVar()
		...
		stateClim1 <- if(GeneralParameters$in.series != 'hourly'){
						if(GeneralParameters$type.series != 'rawts' &
							!GeneralParameters$climato$all.years) 'normal' else 'disabled'
						}else "disabled"
		...
		tkbind(cb.typeseries, "<<ComboboxSelected>>", function(){
			if(!tclvalue(type.series)%in%c('Raw Time Series', '')){
				stateClim0 <- "normal"
		...
		tkconfigure(bt.Extract.Data, command = function(){
			...
			GeneralParameters$date.range$start.hour <- as.numeric(str_trim(tclvalue(istart.hour)))
			...
			GeneralParameters$date.range$end.hour <- as.numeric(str_trim(tclvalue(iend.hour)))
			...
			GeneralParameters$type.series <- switch(str_trim(tclvalue(type.series)),
