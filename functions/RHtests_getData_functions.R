getRHtestsData<-function(gal.params){
	...
	if(single.series=="0"){
		...
		if(!is.null(donne)){
			...
			retval<-list(IDs=donne.id,Lon=lon,Lat=lat,Elv=elv,datdly=dydonne,dydates=dydaty,
			datdek=dkdonne,dkdates=dkdaty,datmon=dmdonne,modates=dmdaty,MissingValue=MissingValue)
		}
	}else{ #1series
