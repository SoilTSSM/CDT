Precip_Merging_ALL <- function(origdir){
	...
	{
		start.date1 <- as.Date(paste0(GeneralParameters$LMCOEF$start.year, '0101'), format = '%Y%m%d')
		end.date1 <- as.Date(paste0(GeneralParameters$LMCOEF$end.year, '1231'), format = '%Y%m%d')
