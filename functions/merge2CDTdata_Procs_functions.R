...
merge2CDTdata <- function(GeneralParameters){
	compareDonneCDT <- function(idc, idate, cdon1, cdon2){
		...
		if(any(!ix)){
			...
			for(j in which(!ix)) cdon1[is.na(cdon1[, j]) & !is.na(cdon2[, j]), j] <- cdon2[is.na(cdon1[, j]) & !is.na(cdon2[, j]), j]
		}else{
			cdon1 <- cdon1
	...
	compareCoordsCDT <- function(idc, chead1, chead2){
		...
		if(any(!ix)){
			...
			for(j in which(!ix)) chead1[is.na(chead1[, j]) & !is.na(chead2[, j]), j] <- chead2[is.na(chead1[, j]) & !is.na(chead2[, j]), j]
		}else{
			chead1 <- chead1
			stn.crd.diff <- NULL
		}
	...
	file1 <- GeneralParameters$file1
	file2 <- GeneralParameters$file2
	File2Save <- GeneralParameters$file2save
	...
	if(ndate1 != ndate2){
		InsertMessagesTxt(main.txt.out, 'The two data have different time step interval', format = TRUE)
		return(NULL)
	}
	...
	if(!is.null(stn.diff.values)){
		outlist <- c(outlist, list('Different Values', stn.diff.values))
	}
	if(!is.null(stn.crd.diff)){
		outlist <- c(outlist, list('Different station coordinates', stn.crd.diff))
	}

	if(length(outlist) > 0){
			containertab <- displayConsOutputTabs(tknotes, outlist, title = 'Different Values')
