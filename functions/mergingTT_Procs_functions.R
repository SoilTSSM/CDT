...
###Downscaling JRA
GlmCoefDownscaling<-function(stn.data,stn.dates,dem.stn,year1,year2,origdir){
...
#	out.tt <- var.def.ncdf("temp", "DegC",xy.dim, NA, longname= "Dwonscaled temperature from reanalysis data", prec="single")
	...
	for (jfl in seq_along(ReanalDataFl)){
...
ExtractReanal2Stn<-function(freqData,ijGrd,nstn,coef.dates,downscaledDir,downPrefix){
	...
	for (xdate in bias.dates){
		rfefl<-file.path(downscaledDir,paste(downPrefix,'_',xdate,'.nc',sep=''),fsep = .Platform$file.sep)
		if(file.exists(rfefl)){
			nc <- open.ncdf(rfefl)
			model <- get.var.ncdf(nc,varid = nc$var[[1]]$name)
			close.ncdf(nc)
			model_stn[which(bias.dates==xdate),]<-model[ijGrd]
		}
	}
	return(model_stn)
}
...
#######################################
calcBias<-function(i,ix0,ix1,stn.data,model_stn){
	stn.tt <- as.numeric(stn.data[ix0, i])
	model.tt <- model_stn[ix1, i]
	ix <- which(!is.na(stn.tt) & !is.na(model.tt))
	nmatch <- length(ix)
	bs <- NA
	if(nmatch >0){
		bs <- sum(stn.tt[ix])/sum(model.tt[ix])
		...
		for (i in 1:nstn){
			...
			for (m in 1:12){
				for (d in 1:endmon[m]){
					...
					ix0<-ixstn[which(as.numeric(substr(stn.dates[ixstn],7,8))==d & as.numeric(substr(stn.dates[ixstn],5,6))==m)]
					ix1<-which(as.numeric(substr(bias.dates,7,8))==d & as.numeric(substr(bias.dates,5,6))==m)
					bias[jdek,i] <- calcBias(i,ix0,ix1,stn.data,model_stn)
					jdek <- jdek+ 1
				}
			}
		}
	...
	if(freqData=='dekadal'){
		...
		for (i in 1:nstn){
			...
			for (m in 1:12){
				for (d in 1:3){
					ixstn<-match(bias.dates,stn.dates)
					ix0<-ixstn[which(as.numeric(substr(stn.dates[ixstn],7,7))==d & as.numeric(substr(stn.dates[ixstn],5,6))==m)]
					ix1<-which(as.numeric(substr(bias.dates,7,7))==d & as.numeric(substr(bias.dates,5,6))==m)
					bias[jdek,i] <- calcBias(i,ix0,ix1,stn.data,model_stn)
					jdek <- jdek+ 1
				}
			}
	...
	if(freqData=='monthly'){
		...
		for (i in 1:nstn){
			for (m in 1:12){
				...
				bias[m,i] <- calcBias(i,ix0,ix1,stn.data,model_stn)
			}
		}
	}
...
############
	...
	if(freqData=='daily'){
		for(m in 1:12){
			...
			for(d in 1:endmon[m]){
				temp<- bias[dek1,]
				tmp <- rbind(tmp,temp,deparse.level = 0)
				dek1 <- dek1+1
			}
			bias.vgm <- as.vector(t(tmp[-1,]))
			bias.df <- data.frame(lon=rep(stn.lon,endmon[m]), lat=rep(stn.lat,endmon[m]), bias=bias.vgm)
			##
			...
			for(d in 1:endmon[m]){
				bs <- bias[dek2,]
			 	...
			 	close.ncdf(nc2)
				insert.txt(main.txt.out,paste("Computing mean bias finished:",paste(meanBiasPrefix,'_',dek2,'.nc',sep='')))
				tcl("update")
			 	dek2 <- dek2 + 1
			}
		}
	...
	if(freqData=='dekadal'){
		for(m in 1:12){
			# Variogrm modeling: Compute variograms for each month(3 dekads)
			tmp <- NA
			for(d in 1:3){
				temp<- bias[dek1,]
				tmp <- rbind(tmp,temp,deparse.level = 0)
			...
			bias.vgm <- as.vector(t(tmp[-1,]))
			bias.df <- data.frame(lon=rep(stn.lon,3), lat=rep(stn.lat,3), bias=bias.vgm)
			##
			vgm_model <-Variogrm_modeling(bias.df,stn.lon,stn.lat)
			for(d in 1:3){
				bs <- bias[dek2,]
	...
	if(freqData=='monthly'){
		for(m in 1:12){
			bias.vgm <- as.vector(t(bias[m,]))
			bias.df <- data.frame(lon=stn.lon, lat=stn.lat, bias=bias.vgm)
			##
			vgm_model <-Variogrm_modeling(bias.df,stn.lon,stn.lat)
			bs <- bias[m,]
			...
			grd.bias<-interpBias(stn.lon,stn.lat,bs,newlocation.merging,max.nbrs,vgm_model,nlon0,nlat0)
		 	...
		 	put.var.ncdf(nc2,grd.bs,grd.bias)
		 	close.ncdf(nc2)
			insert.txt(main.txt.out,paste("Computing mean bias finished:",paste(meanBiasPrefix,'_',m,'.nc',sep='')))
			tcl("update")
		}
	}
...
ComputeRegCoeff<-function(stn.data,stn.dates,coef.dates,dem.stn,model_stn,origdir){
	years<- as.numeric(substr(stn.dates,1,4))
	...
	for (m in 1:12){
		...
		if(!is.na(coef[m,1]) & is.na(coef[m,2])){
			...
			insert.txt(main.txt.out,paste('Mean and standard deviation of',format(ISOdate(2014,m,1),"%B"),
			'are NA'),format=TRUE)
			return(NULL)
		}
...
AjdReanalMeanBias<-function(freqData,istart,iend,dem,xy.dim,nlon0,nlat0,
...
AjdReanalpmm<-function(freqData,istart,iend,stnDatas,ijGrd,dem,xy.dim,nlon0,nlat0,
coefReg,downscaledDir,adjDir,downPrefix,adjPrefix){
	...
	for (jfl in seq_along(downDataFl)){
		...
		#q1 <- quantile(stn.tt,0.0005,na.rm=T)
		#q2 <- quantile(stn.tt,0.9995, na.rm=T)
...
MergeTemp<-function(mrgParam){
	...
	tcl("update","idletasks")
	for (jfl in seq_along(adjDataFl)){
		...
		insert.txt(main.txt.out,paste("Dekadal temperature merging finished successfully:",paste(mrgPrefix,'_',mrg.dates[jfl],'_',mrgSuffix,'.nc',sep='')))
		tcl("update")
	}
	#return(0)
}
