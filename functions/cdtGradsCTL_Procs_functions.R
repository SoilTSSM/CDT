...
grads_create.ctl_Procs <- function(params){
	daty1 <- as.Date(paste(params$date$year1, params$date$mon1, params$date$day1, sep = "-"))
	daty2 <- as.Date(paste(params$date$year2, params$date$mon2, params$date$day2, sep = "-"))
	# do.call(paste, c(params$date[c('year1', 'mon1', 'day1')], list(sep = "-")))
	filename0 <- gsub("%Y|%M|%D|%T", "%s", params$nc$format)
	filename <- gsub("%D", "%d2", gsub("%M", "%m2", gsub("%Y", "%y4", params$nc$format)))
	DSET <- paste("DSET", file.path(params$nc$dir, filename))

	if(params$tstep == "daily"){
	...
	ncpath <- file.path(params$nc$dir, ncfiles)
	...
	ncdataInfo <- getRFESampleData(params$nc$sample)
	...
	nc.varid <- ncdataInfo$rfeVarid
	lon <- nc$var[[nc.varid]]$dim[[ncdataInfo$rfeILon]]$vals
	lat <- nc$var[[nc.varid]]$dim[[ncdataInfo$rfeILat]]$vals
